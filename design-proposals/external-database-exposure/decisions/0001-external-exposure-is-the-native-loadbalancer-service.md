...
- The migration cost fell only on clusters tracking `main` that had set `publishing.exposureClass`: provision an address pool, switch to `publishing.loadBalancerClass`, delete any orphaned `cozystack-<class>` pool by hand. The v1.6.0 release itself never carried the kinds.
