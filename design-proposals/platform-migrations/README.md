...
**The value is structured, not a sentence.** Four things parse it — the runner, the linter, the E2E assertion and compaction — and a positional string is how four slightly different parsers come into existence. It is JSON: `jq` is installed in the migrations image, so the runner can both read and build it without string surgery. `state` is one of `ok`, `legacy`, `revoked`, `skipped`, `failed`, `retry`; `batch` records where §3 placed the migration; `image` records what actually executed, which matters because a background migration pending since 1.7 runs under 1.8's image; `attempts` counts executions rather than reconciles. A failure additionally carries `reason` and a human `message`. The exact field set will move during implementation and that is fine — what is settled here is that the value is parsed, not read.
...
**Nothing is retried automatically, in either tier.** A migration that fails under `on-error=warn` is recorded `failed` with its reason, message, image and attempt count, and is not attempted again on the next upgrade or the next reconcile. This is the current behaviour made explicit rather than a new restriction — today nothing retries either — and the reason to keep it is that an automatic retry of work nobody is watching is how a failure becomes permanent background noise. The upgrade proceeds; the record is the report.

Re-running is therefore an operator action, and it needs a handle that does not destroy the evidence. Setting an entry's `state` to `retry` is that handle: the runner and the operator both treat `retry` as pending, and the next execution rewrites the entry with a fresh outcome, an incremented `attempts`, and the previous failure preserved in the record. Deleting the ledger key would also make the migration pending, but it throws away the one account of what went wrong, so the linter and the documentation both point at `retry` instead. Because §5 requires every migration to be safe to run twice, this is safe for any of them, not for a blessed subset.
...
- Operator re-runs a failed migration → sets that entry's `state` to `retry`; the next pass treats it as pending, executes it, and rewrites the entry with a fresh outcome and `attempts` incremented.
...
**Controller (envtest).** Pending computation including revoked, Job creation, ledger patch on success, a failing Job surfacing as `Ready=False` with a distinct reason and **not** being recreated on the next reconcile, and an entry moved to `retry` being picked up exactly once.
