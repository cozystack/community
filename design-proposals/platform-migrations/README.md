...
packages/core/platform/images/migrations/migrations/
  ...
  order.d/           # release-assigned execution order (§3); generated at build, not in git
    00001-v1.6.0
    00002-v1.6.3
...
> This has already happened in tree. [#3315](https://github.com/cozystack/cozystack/pull/3315) was opened on 2026-07-15, before [#3379](https://github.com/cozystack/cozystack/pull/3379) and [#3406](https://github.com/cozystack/cozystack/pull/3406), and merged on 2026-08-26, after both. It shipped as migration `56` while those two took `54` and `55`. Under any authoring-time key — a date, a name, a hash — the relative order of `56` against the other two depends on which cluster is asked, and no dependency was declared because the author of #3315 never knew the other two would land first.

**Is a total order needed at all?** Not proven, and this section should say so rather than let the machinery imply it. No migration in tree states an order requirement against another migration: the order-sensitive cases in §2 are ordered against the *chart* — `56` must create the child HelmReleases before the kubernetes chart stops rendering worker objects, `46` must patch `spec.version` before the chart's `fail()` guard sees v1.30 — and not against a sibling migration. What is demonstrably broken today is the other failure, a late-merged migration treated as already applied ([#3534](https://github.com/cozystack/cozystack/issues/3534)), and the slug-keyed ledger of §4 fixes that on its own with no ordering at all.

So the batch mechanism is insurance, not a proven requirement, and it is kept on the strength of what it costs rather than what it has already prevented: an author picks no number, touches no file, waits on no slot and adds no line to any PR, because §3 is derived from tags the release already produces. The day a pair does need a declared order, the alternative to having this is finding out on a cluster. If that trade stops looking worth it, the thing to delete is this section, and §4 keeps working without it.
...
```
# batch 00003, first shipped in v1.7.0-rc.1, generated by hack/generate-migration-order.sh
clickhouse-keeper-pvc-labels
vm-pool-adopt
etcd-crds-precreate
```
...
**Within a batch, nothing needs assembling.** Two migrations first shipped by the same release are seen by every cluster in one pass off one image, so the agreement requirement says nothing about their relative order and any deterministic rule satisfies it. That collapses the build step to *reading which tag introduced each slug*: no reconstruction of merge order, no rank counter, no git archaeology. The rule is alphabetical order of the slugs, then reordered where a `requires` in the same batch demands it.

**The prefix invariant, which is the whole guarantee in one check.** Because batches only ever append, each release's manifest is a literal prefix of the next one's. Deriving it from tags makes that structural rather than asserted: a batch is fixed by the first tag whose tree holds the slug, and that tag does not change afterwards, so there is no file for a later step to edit and no lint asserting that the first N lines did not move. One check is still worth running, because it catches the one thing that would break the property — a tag that moved: regenerate the previous release's manifest and compare it against the head of this one.

**What the build does.** `hack/generate-migration-order.sh`, run from the tagged checkout before the image and the chart are packaged. `build-release.yaml` already checks out with `fetch-depth: 0` and `fetch-tags: true`, so the history it needs is present:

1. Walk this line's tags in release order and `git ls-tree` the two tier directories at each.
...
Deriving from tags removes the slot instead of building one. Tags are immutable and the release workflows treat them that way, so a batch assignment cannot be edited, lost, or raced, and the rc case resolves correctly: rc.1's tree fixes the batch of everything it carries, and rc.2 assigns a later batch only to what rc.1 did not.
...
What the edge does depends on where its target already sits. If the target shipped in an earlier release, the dependency is satisfied before the runner starts and `requires` acts purely as a check. If neither has shipped yet they land in the same batch, and the generator uses the edge to order them within it. Either way it is resolved when the manifest is generated, so the runner parses no headers and evaluates no dependencies. What remains is the value worth keeping: a declared dependency CI can verify, which fails loudly instead of running in the wrong order.

**What is guaranteed, and what is not.** Every cluster upgrading along one release line runs any two migrations in the same relative order. Cross-line, order may differ for exactly one class: a migration backported to a maintenance branch, which is assigned a batch on that branch and a later batch on `main`. Trying to reconcile the two would mean splicing an entry into the middle of a batch `main` already shipped, which is precisely what the prefix invariant forbids — so it is not reconciled. That divergence is safe by the same reasoning that justified the backport: it is the maintainer's assertion that the migration is correct on a branch missing everything in between. CI reports the affected pairs by generating the manifest for each branch and diffing them, so the assertion is explicit rather than assumed. Today the same divergence exists and is invisible — [#3534](https://github.com/cozystack/cozystack/issues/3534) was found by hand.
...
2. `.github/workflows/backport.yaml` cherry-picks it onto `release-1.5`: same path, same slug, same bytes. **The author does nothing beyond the cherry-pick** — nothing to renumber, nothing to rename, no manifest to edit, because no tag carries the slug yet and there is no manifest in the tree to edit in any case.
3. `v1.5.4-rc.1` is cut from `release-1.5`. That branch's manifest gains a batch containing `kubeadm-keep-policy`.
4. `v1.7.0-rc.1` is cut from `main`. Main's manifest gains its own batch, also containing `kubeadm-keep-policy`, because that is the first tag on `main` whose tree carries it.
...
What differs is its position relative to `monitoring-move`, a 1.6 migration the 1.5 line never carried. Reconciling that would mean splicing the slug into `main`'s manifest at its 1.5.4 position — inside batches already shipped, which the prefix invariant forbids. So it is not reconciled, and the divergence is the backport's own premise: cherry-picking to `release-1.5` asserted the migration is correct on a branch holding none of 1.6.
...
**The value is structured, not a sentence.** Four things parse it — the runner, the linter, the E2E assertion and compaction — and a positional string is how four slightly different parsers come into existence. It is JSON: `jq` is installed in the migrations image, so the runner can both read and build it without string surgery. `state` is one of `ok`, `legacy`, `revoked`, `skipped`, `failed`; `batch` records where §3 placed the migration; `image` records what actually executed, which matters because a background migration pending since 1.7 runs under 1.8's image; `attempts` counts executions of that migration on this cluster and has two sources, the operator's capped background retry and the manual re-run, both in §7 — an entry is written with `1` and grows from there, which makes "this has already been tried three times" visible before anyone tries a fourth and is what the background cap is counted against. A failure additionally carries `reason` and a human `message`. The exact field set will move during implementation and that is fine — what is settled here is that the value is parsed, not read.
...
**The watermark cannot hide a migration that still ships.** `complete-through` is the highest batch every one of whose entries has reached a terminal state — recorded in the ledger, or flagged `retired` (§11). It is a **batch number**, which is assigned by the first release whose tree carries the slug and is therefore always the newest one at that point, so a migration merged years after it was written still receives the newest batch. There is no value a newly merged migration can be given that sits below an existing watermark. Contrast a date: a July-dated file merging after the watermark passed July is below it and is treated as applied, which is exactly the silent skip this proposal exists to remove. golang-migrate shows the failure in a shipped library: its single `version, dirty` row makes a late lower-numbered migration permanently unreachable, with no error and no override.
...
A `failed` entry is terminal for the watermark's purposes and does not pin it. Since a background failure is retried only up to its cap and a blocking warn failure is not retried at all (§7), pinning the watermark on a failure would stall retention on that cluster indefinitely for a migration nothing is coming back to. The failure stays visible instead, in its own entry, which is never compacted at any age.
...
There is no sorting, no dependency resolution and no version comparison at runtime: §3 settled all three at build time. That is what keeps the runner to a flat walk over a file, and it is why `requires` never has to be parsed here.

Two conditions are fatal and must stay fatal. A slug present on disk but in no batch means the manifest was not generated for this build, and a slug in a batch but absent from the image means the packaging is wrong; both abort before anything executes. Downgrading either to a warning reintroduces the silent skip.
...
**A recorded failure is retried only where something is watching.** A `pre-apply` migration that fails under `on-error=warn` is recorded `failed` with its reason, message, image and attempt count, and is not attempted again on the next upgrade. The upgrade proceeds and the record is the report, which is the current behaviour made explicit rather than a new restriction.

The background tier is different, and treating the two alike was wrong. That tier already has something watching it: the operator reconciles it and reports a `Ready=False` condition with a per-failure reason, so a failure there is surfaced rather than buried in a ConfigMap nobody opens. And on the air-gapped and slow-moving clusters this proposal counts as real when it rejects a kubeadm-style skip ban, "the record is the report" means nobody reads it for months. So the operator retries a failed background migration with exponential backoff up to a small fixed cap, incrementing `attempts` on each try; after the cap the entry stays `failed` and nothing retries it again. What the original objection rejects is unbounded retry of unwatched work, and a capped retry of watched work is not that. §5 already requires every migration to be safe to run twice, so this needs no per-migration opt-in.
...
**When an operator would do this.** A `failed` entry means the work did not happen and whatever it was fixing is still broken. The record says which migration, why it failed and what it said, so the decision is: remove the cause — a tenant that was mid-delete, a throttling apiserver, an object something else held — and re-run. There is no value in re-running before that, which is why the background retry is capped rather than indefinite and why the blocking tier does not retry a warn failure at all.

Failures need a surface, since past the cap nothing chases them. For the background tier the operator already reports one, as a `Ready=False` condition with a per-failure reason. For the blocking tier the ledger is the surface: a `failed` entry stays in the ConfigMap, uncompacted at any age, and `kubectl get configmap -n cozy-system cozystack-version -o yaml` is the report an operator reads.
...
1. Rename `migrations/1..56` to slugs describing what each does, so `1` becomes `mariadb-operator-secrets`. Order is not encoded in the names and does not need to be: batch `00000` lists all fifty-six slugs in their original integer order, which is by construction the order every existing cluster already ran them in. It is the one batch the generator does not derive — the integer set predates the slug scheme, so no tag's tree carries those slugs in that order — and it is supplied instead by the committed `legacy-map` of step 2, which already holds exactly that ordering. That batch sorts before every release batch, so the converted set keeps its position in the global sequence exactly. Its internal order is never executed in practice: every existing cluster has already run the integers, and a fresh install seeds them (§7).
...
Deleting a migration means deleting the script. The `retired` flag is not written by hand and cannot be: §3 derives it, from a slug present in an earlier tag's tree and absent from this one. What the generator emits is:

```
# batch 00001, first shipped in v1.6.0
mariadb-operator-secrets    retired
flux-tenants-cleanup        retired
```

The line stays. It costs about forty bytes and it is what lets the runner tell a retired migration from a slug it has never heard of — the second of which is a packaging error that must abort. The invariant holds by construction rather than by lint: **every manifest entry satisfies exactly one of {the script exists, the entry is flagged `retired`}**, because the generator emits an entry only for a slug some tag carried and flags exactly the ones this tree no longer has. `retired` is not reversible either, since restoring the slug would collide with the uniqueness check §5 runs against tag history. That is what makes it impossible for the watermark to cover something that still ships, without relying on anyone comparing dates.
...
- `requires` names a slug that does not exist, names one in a later batch, or the graph has a cycle → the **build** fails. The runner never evaluates `requires`, because §3 resolved it into the manifest.
- Slug on disk but in no batch → the manifest was not generated for this build. Runner aborts before executing anything, and the render gate fails for the same reason.
- Slug in a batch, not flagged `retired`, and absent from the image → packaging error. Runner aborts.
- Two migrations first shipped by the same release with no `requires` → both run in slug order, identically on every cluster, because every cluster sees that batch in one pass off one image.
...
- Background Job fails → recorded `failed` with reason and message, surfaced as a `Ready=False` condition with a distinct reason on the operator, and retried with backoff until `attempts` reaches the cap, after which it is **not** retried again. The upgrade is not blocked at any point.
...
**E2E (upgrade lane).** [#3276](https://github.com/cozystack/cozystack/pull/3276) currently asserts the stamp reached `migrations.targetVersion`. Its replacement, required before Phase 5 retires `targetVersion`, is an **empty-pending assertion**: every slug in the manifest of the tree under test that is not flagged `retired` must have a matching `m.<slug>` ledger key whose outcome is terminal — `ok`, `legacy`, `revoked` or `skipped-fresh-install`. Because a fresh install seeds every slug it ships, the lane must additionally assert that the slugs introduced between the base release and the release under test carry `ok` specifically, or it passes trivially on a clean install. Both sides are cheap — generate the manifest for the tree under test, read the ConfigMap's keys — and it is strictly stronger than the integer compare, because it proves each individual migration reached a terminal state rather than that a counter moved. The background half is asynchronous, so it polls; any other outcome fails the lane and names the offending ID.
...
**Phase 1 — engine and contract.** Dual-mode runner, `record_migration`, set-difference render gate, header parser and `on-error` handling, `hack/lint-migrations.sh`, `hack/new-migration.sh`, `lib/` helpers including the fleet-iteration one, fresh-install ledger seeding (§7), `hack/generate-migration-order.sh` wired into the image and chart builds, and five CI guards (integer freeze, content immutability on every release branch, slug grammar plus uniqueness against tag history, no `order.d/` in the source tree, and the regenerate-and-compare prefix check against the previous release). Every migration is `pre-apply`, so runtime behaviour is unchanged. The linter enforces on `pre-apply/` and `background/` only; the 56 frozen integers are grandfathered.
...
**Phase 5 — legacy conversion and retention.** Commit `legacy-map`, implement seeding, delete the legacy pass and `targetVersion`. Rename the set to slugs and supply batch `00000` to the generator from `legacy-map`. Depends only on Phase 1 plus at least one shipped release of soak, so the slug path is exercised before the seeding path is added; independent of Phases 2–4. The retention floor is introduced in the first release that actually deletes a migration, not before.
...
**Ordering — a declared dependency graph as the order.** Rejected, with the worked reason in §3. Django is the honest version of this design: it disclaims filename ordering outright and never promises a stable order for pairs with no declared edge. `requires` is kept here, but as an assertion resolved when the manifest is generated rather than as the ordering mechanism.

**Ordering — the release step renames files, `goose fix` style.** goose renames timestamped development files to sequential integers in CI once they are ready to ship. Rejected because the filename is the ledger key here, so a rename would rewrite cluster state and break cross-branch identity. A manifest derived at build time gets the same effect without touching the name.
