...
- **PR [#3002](https://github.com/cozystack/cozystack/pull/3002)** (`vm-instance` `firmware` field) is a **merge-order dependency**: the controller writes `spec.firmware` onto created VMInstances, but the field exists nowhere in the vm-instance schema on main, so structural pruning drops it and UEFI guests silently land as BIOS. #3002 must land first. Until it does, the interim behaviour is explicit rather than best-effort: the controller reads the bootloader from the Forklift-built `VirtualMachine` it already parses, and **fails an EFI guest per-VM with a message naming the reason**. Importing a machine that cannot boot and reporting success is precisely the failure this proposal's own validation philosophy — surface it on status before anything is created — exists to prevent.
