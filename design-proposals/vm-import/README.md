...
- No part of the import path requires a privileged namespace, a privileged pod, or `seccompProfile: Unconfined`; guest conversion, when enabled, costs exactly one narrow syscall through a `Localhost` profile.
...
**The conversion path does not need a privileged namespace either — it needs a `Localhost` seccomp profile, which `baseline` and `restricted` both accept.** The blocker is narrower than "virt-v2v is privileged": libguestfs inside the conversion image starts `passt` for the appliance's network, `passt` calls `clone(CLONE_NEWUSER)`, and `RuntimeDefault` blocks that syscall without `CAP_SYS_ADMIN`. `passt` appears nowhere in Forklift's own tree — it comes from the image — so this cannot be switched off from Forklift's Go code. What *can* change is the profile: on Forklift v2.11.5 the conversion pod already selects `Localhost` with `profiles/unshare.json` when `settings.Settings.OpenShift` is true and falls back to `RuntimeDefault` otherwise (`pkg/controller/plan/kubevirt.go:2306-2317`), and `OPENSHIFT` is autodetected false on any non-OpenShift cluster. On Forklift's current `main` the branch has been **removed entirely** — `RuntimeDefault` unconditionally, zero occurrences of `unshare` — so upgrading Forklift does not fix this and upstream has no path for it today: issue #4491 is open, with reporters on RKE2/Harvester and on Talos v1.11.5 hitting the same `passt … Couldn't create user namespace: Operation not permitted`, after PR #1943 traded a working profile for a pod that merely starts on Kubernetes.
...
- **Forklift side:** an upstream knob that selects the conversion pod's seccomp profile independently of OpenShift detection, defaulting to today's behaviour so no existing user changes. The change follows Forklift's own convention for tunables exactly — a const plus field plus loader in `pkg/settings/settings.go`, consumption at the branch in `pkg/controller/plan/kubevirt.go`, an ansible default in `operator/roles/forkliftcontroller/defaults/main.yml`, an env var in `templates/controller/deployment-controller.yml.j2`, and a snake_case CRD field alongside the existing `controller_log_level` and `feature_ui_plugin`. Cozystack sets it in `packages/system/forklift/templates/forklift-cr.yaml`. Until that lands, a label-scoped mutating webhook on `forklift.app=virt-v2v` pods setting `Localhost` (never `Unconfined`) is a sound interim, shipped in the `forklift` package.
...
- **Privilege containment:** nothing in the tenant path is privileged. The CDI importer pod and the `vddk-validator` Job both satisfy `restricted`, verified against Forklift v2.11.5 and CDI v1.64.0 (§4). Conversion, when it is enabled later, gains exactly one narrow syscall (`unshare`/`clone(CLONE_NEWUSER)`) through a `Localhost` seccomp profile — never `Unconfined`, never a privileged namespace, and syscall filtering stays on for everything else.
...
3. **Phase 3 — guest conversion, on its own track.** Land the upstream Forklift seccomp knob (§4), ship the narrow `unshare.json` through Talos `machine.seccompProfiles`, and enable conversion inside the tenant namespace under `baseline`. This phase is deliberately independent of Phases 1 and 2: it unblocks guests without virtio drivers, and it can slip without affecting anything already shipped. If upstream declines the knob, the label-scoped mutating webhook is the fallback and lands here instead.
...
- **Upstream seccomp knob.** Whether Forklift takes a setting that selects the conversion pod's seccomp profile independently of OpenShift autodetection. The situation favours trying: the `Localhost`/`unshare.json` branch that existed in v2.11.5 has been deleted on `main`, issue #4491 is open with reporters on RKE2/Harvester and Talos, and the change follows Forklift's own tunable convention across five files. Recommended default: open the upstream PR against `main` with the default preserving current behaviour, and hold the interim webhook in reserve rather than shipping it first. Conversion is Phase 3 either way, so nothing earlier waits on the answer.
