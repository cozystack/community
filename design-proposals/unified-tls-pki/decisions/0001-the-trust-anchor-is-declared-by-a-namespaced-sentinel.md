...
- The dot in the canonical name rests on one assumption the platform does not enforce — that no per-release Secret suffix in use ends with `.tenant-ca`. Both halves of a release name are dot-free by validation, so a suffix can spell the canonical name only by supplying that dot itself; a dotted suffix ending anywhere else cannot reach it, which is why the dotted names already in use under [#2729](https://github.com/cozystack/cozystack/pull/2729) are no threat to it. The enforced legs are the application name and the release prefix; the residue is covered by refusing, rather than adopting, a Secret already sitting at the canonical name.
