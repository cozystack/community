...
- **Tunnel entry point — native `Service type: LoadBalancer`, no blocking dependency.** The tunnel's own external entry point (the UDP listener the remote peer dials) is published through a native `Service type: LoadBalancer` + `loadBalancerClass` from an admin-provisioned pool — the primitive that replaced the withdrawn `network.cozystack.io` exposure group (`ExposureClass` / `ServiceExposure`) in [cozystack/cozystack#3218](https://github.com/cozystack/cozystack/pull/3218). A native `Service` carries protocol per port and the LB implementations already in use handle UDP (IKE/NAT-T 4500, WireGuard), so Phase 1 carries **no exposure blocker**. Two consequences worth stating up front, neither a blocker: the endpoint consumes one address from the tenant's LB pool (quota), and the address cannot yet be pinned explicitly by the tenant (it is pool-allocated). This proposal commits to the **responder model** for Phase 1 — the remote site dials in to that address (see the initiator-vs-responder note below).
- **Initiator vs responder.** Phase 1 is deliberately **responder-only** (remote dials in). The alternative — the gateway as **initiator** (dial-out), which needs no `Service`, no LB, and no ingress address at all — is real and attractive (a site-to-site tunnel is symmetric once up, and the forced-UDP encapsulation this design already mandates *is* NAT-traversal mode), but it trades the inbound exposure for a **stable, tenant-owned egress identity**: a dial-out gateway leaves via the cluster egress masquerade wearing the *node's* shared, unstable address, which remote gear routinely pins in its IKE config and firewall ACLs. That egress identity does not exist yet (tracked in [#35](https://github.com/cozystack/community/pull/35)), and an LB `Service` structurally cannot express it. Initiator/either-end operation is therefore **deferred future work on the gateway, gated on stable per-tenant egress IPs** — folded together with the per-tenant egress IP item below, not committed in Phase 1.
...
| Security posture | fully contained | gateway port security disabled; Cilium destination policy + guest firewall required |
...
flowchart TB
    ...
    xp["LoadBalancer Service<br/>(loadBalancerClass, UDP)"]
...
**Tunnel entry point.** Published via a native `Service type: LoadBalancer` + `loadBalancerClass` (UDP) from an admin-provisioned pool — one shared external-address contract for both apps. Phase 1 is responder-only (remote dials this address); see [Scope](#scope-and-related-proposals) for the deferred initiator model.
...
- **Portal-side image / cloud-init synchronization.** The tenant dashboard (portal) that surfaces the app must keep the referenced image and the generated cloud-init in lock-step with the platform-published artifact: when a new gateway image is pinned, the portal has to advance the app's image reference and regenerate the first-boot cloud-init together, so the effective boot config always matches the image it boots from and a tenant is never left booting a new image with stale cloud-init (or vice versa). The exact sync mechanism (ApplicationDefinition default vs. controller-reconciled) is an implementation detail, but the invariant — image ref and cloud-init advance atomically — is a design requirement.
...
1. The remote peer sends tunnel traffic to the gateway's `LoadBalancer` UDP endpoint; the VM decrypts the tunnel.
...
**Topology-advertising apps.** Protocols where a client bootstraps to one endpoint and is then handed the members' own addresses to connect to directly — Kafka (`advertised.listeners`), MongoDB replica-set/sharded, Redis Cluster, Cassandra/Scylla, Elasticsearch with sniffing — are **not** transparent under plain NAT of the bootstrap endpoint: each member needs its own tunnel-reachable address that the app also advertises. Under `site-gateway` this means explicit per-member mapping (a distinct tunnel-side address:port per member, plus the app configured to advertise it); the managed-app operators already do this kind of thing for public exposure, but a native `LoadBalancer` `Service` exposes a single address per named listener, not one per broker/replica, so it does not directly cover per-member addressing today. The transparent path for this class is therefore **`site-router` (routed)** — the members' real addresses are directly reachable, so discovery works as long as members advertise a routable ClusterIP. Folding per-member NAT into a tunnel-backed exposure mechanism is a possible future unification, not a solved design (see [Open questions](#open-questions)). Single-endpoint protocols (Postgres, MySQL, plain Redis, AMQP, ClickHouse client, HTTP/gRPC) are transparent under NAT and need none of this.
...
- **Phase 1 — `site-router` (routed), IPsec backend**: the routed dataplane plus its CNI-mediation controller (remote-CIDR validation against the full deny-set, routes annotation, gateway-only `port_security` relaxation), with **MSS clamping**, **tunnel-state observability** (SA up/down, rekey, counters), and the source, destination, metadata, public-forwarding, and management-API guards described in [Security](#security). Full relaxation is accepted because kube-ovn v1.15.10 cannot emit CIDR-scoped allowed addresses; the resulting arbitrary-source injection risk is explicit, and the source allow-list is enforced in the guest. Phase 1 is not accepted until the negative security tests pass. This mode is validated in the existing production implementation, which is why it leads.
- **Phase 2 — `site-gateway` (NAT): a deliberate design re-open, not a committed plan.** The two-app split is settled, but `site-gateway`'s values schema and controller are **not frozen by this proposal** — they are reopened for design once Phase 1 is implemented and we have collected real operational pain from the routed mode. The inbound/outbound DNAT/SNAT machinery is prototype-validated (with forced UDP encapsulation) and fully contained (SNAT, so no `port_security` relaxation at all), so the mechanism is sound; what Phase 2 revisits is the concrete surface, informed by Phase 1 rather than pre-committed here.
...
- **Phase 4 — the rest**: HA (Service-fronted active/passive); per-tenant egress IP, and — riding on it — the **initiator / either-end tunnel model** (dial-out and responder-or-initiator), which is unblocked only once a stable tenant-owned egress identity exists (see [Scope](#scope-and-related-proposals) and [#35](https://github.com/cozystack/community/pull/35)).

Ordering leads with routed because it is the primary requested mode and the one already proven in production; the shared foundation is built once and both apps sit on it. MSS clamping, guest source filtering, management isolation, gateway-specific Cilium denies, and the negative security suite ship in Phase 1 alongside the necessary port-security relaxation.
...
- **Tunnel exposure via native `LoadBalancer`.** The tunnel endpoint is a native `Service type: LoadBalancer` + `loadBalancerClass` and carries **no Phase 1 blocker** (see [Scope](#scope-and-related-proposals)); the open detail is how the pool-allocated address interacts with the tenant's LB pool and quotas, and whether tenants need to pin the address explicitly (not possible today) rather than take a pool allocation.
- **Topology-advertising apps under NAT.** Whether per-member advertised addressing for Kafka / Mongo / Redis-Cluster / Cassandra under `site-gateway` is worth a tunnel-backed per-member exposure mechanism (needs per-member granularity a single `LoadBalancer` `Service` does not have today) or is simply left to `site-router`.
- **NETMAP-style 1:1 subnet mapping (NAT future work).** Under `site-gateway`, an alternative to per-target host:port DNAT is a **NETMAP-style 1:1 subnet mapping** (map a remote CIDR onto a tenant-side CIDR preserving host offsets), which would give whole-subnet reachability under NAT while keeping relative addressing — closer to what `site-router` offers, but CNI-agnostic. Recorded as NAT future work, to be evaluated in the Phase 2 re-open, not committed here.
...
- **Scoped port security for `site-router` (follow-up hardening).** kube-ovn v1.15.10 allowed-address-pairs accept host IPs only, while OVN itself accepts `MAC IP/mask`; Phase 1 therefore cannot scope a whole routed prefix through the supported kube-ovn API. Track upstream CIDR support (or contribute it), then replace full relaxation with declared-prefix scoping without removing the guest source filter or negative tests.
