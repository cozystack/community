...
- Retirement of the static `super-admin` break-glass kubeconfig, and therefore its phase. The OIDC administrative route already conforms and is classified in phase 1, so what is left is a scope decision for the API and code owners: a `system:masters` certificate cannot be de-authorized and cannot be revoked individually, so rotating the tenant cluster's client CA is the only mechanism available, and it has to reissue the platform's own certificates in the same operation to keep remote Flux apply and the in-cluster platform components working. What `strictOnly` does with an `oidc.mode: None` cluster until that ships is part of the same decision.
