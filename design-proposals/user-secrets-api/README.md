...
- **Title:** `Private credential inputs, one-time disclosure, and managed credential regeneration`
...
These are the choices proposed for review, not claims of maintainer approval or completed implementation. Implementation decision records will be added under this proposal when a choice is settled or changes, following the [repository process](../README.md#decision-records).
...
| One-time disclosure | One collection entitlement per generated account version, shared across all authorized recipients. Discovery never contains protected fields. | Prevents a second API retrieval through another client or identity. See [collection](#one-time-collection). |
...
| Rights and hierarchy | Separate discovery, supply, use, collection, regeneration, and delegation. No implicit ancestor credential authority. Organization policy caps all grants. | App administration and a guessed Secret name do not authorize disclosure or use. See [authorization](#authorization-and-delegation). |
| Audit | Durable lifecycle evidence plus Metadata-level API audit; export structured events through ordinary log collectors and provide authorized service-history subscriptions. | API access records alone cannot prove engine activation or identify a user behind a backend. |
| Coverage | First wave: PostgreSQL/MariaDB users, COSI-backed bucket keys, and private external PostgreSQL replication authentication. Later waves and unsupported classes are listed explicitly. | Covers both scalar and operator-issued material without pretending every engine has identical capabilities. |
| Compatibility | Strict by default for new supported installations. Existing installations get a finite two-release migration window, followed by explicit regeneration before strict attestation. | Previously exposed bytes cannot become undisclosed by changing a label. See [compatibility](#upgrade-and-rollback-compatibility). |
...
| Dashboard and automation | The existing secret list already transfers data before the Eye button is pressed. Terraform credential outputs retain values in state. Both require a changed client contract. |
| Audit | Cozystack does not provide the complete policy-and-export pipeline required here. Talos supplies a default node-local Metadata audit log; that is neither centralized export nor engine lifecycle evidence. Other distributions need explicit configuration. |
...
Application creation declares accounts and causes one initial generation request per new incarnation. Account creation must pass the same scoped generation-authority check as an explicit request; a generic app-edit grant alone cannot authorize a new credential or consumer. The server records the initiating identity for this implicit initial request. Editing unrelated options, schema validation, dry-run, Helm rendering, and GitOps reapply do not generate new versions. Mutations require expected-version preconditions. No mutable counter in app values acts as a rotation trigger.
...
Initial creation and regeneration return metadata immediately. Once activation is verified, a new version becomes collectible for 24 hours. Creation, polling, and workload delivery do not consume this entitlement. Expiry leaves the active credential usable by its workloads but unavailable for collection; a recipient who missed the window must request an authorized regeneration. An administrator cannot reopen an expired or spent version.
...
`CredentialBinding` is a metadata-only object tying source record UID, account fields, destination workload UID, application UID, and a controller-issued destination identity to one allowed purpose. Creating it requires both source-use authority and destination-management authority. The first adapter delivers inside the same tenant namespace to a managed workload through a fixed-layout Secret. Other namespaces, clusters, arbitrary webhooks, VM guests, and external stores return unsupported until their adapters and policies ship.

Local delivery follows the active version; version pinning is not supported initially. The controller updates a private destination Secret and records `deliveredVersion`. An adapter that verifies a fresh connection records `adoptedVersion`; one that cannot observe reload reports `AdoptionUnknown`. File refresh, environment rollout, and connection-pool reuse are separate states. Regeneration completion describes source authentication, not universal consumer adoption. Removing a binding stops new copies within five seconds and removes controller-owned destinations when reachable, while reporting copies or sessions that cannot be withdrawn.
...
| Managed Kubernetes admin kubeconfig, long-lived service-account token | Sensitive, withheld from strict ordinary tenant routes initially | Self-service collection/regeneration disabled until issuer-specific identity, expiry, replacement, and revocation are proven | Kubernetes/identity maintainers own follow-up. Certificate download does not make revocation equivalent to password rotation; CA rotation may affect other identities. Existing break-glass use is an audited platform-admin procedure outside tenant collection. |
...
Normal export and alert delivery must occur within 60 seconds. Alert at five minutes of export lag. At ten minutes of lag, or when durable storage reaches its reserved capacity threshold, block new supply, issuance, collection, delivery, and regeneration admissions with `503 AuditUnavailable`. Once an engine mutation has started, verification and recording its outcome take priority using reserved outbox/journal capacity; do not abandon it because export stopped. If outcome storage itself fails, retain candidates, stop further mutation, and report uncertainty until durable evidence is recovered. Revoke follows the same durable admission rule. Emergency platform-admin intervention is a separately audited operational procedure and cannot be reported as fully audited API success during a journal outage.
...
Initial defaults are 64 KiB decoded material per input/version bundle, 32 declared fields per bundle, 100 accounts per application, and 1,000 credential/input records per tenant. Allow six sensitive operation admissions per minute per tenant with a burst of ten, one mutation per account, and at most 100 active appliers fleet-wide. Queue admission is bounded and returns `429` before accepting work it cannot retain. Metadata discovery has a separate budget, so denied collection does not exhaust an account's sole entitlement or prevent health reads.
...
New installations enable strict behavior for conforming adapters. Unsupported secret-bearing services are unavailable there until converted. Existing installations remain in explicitly reported `Legacy` mode through A and B, with a deadline at the next platform minor release after B. At that release the upgrade blocks tenants that have not completed their migration; it does not silently keep repeatable reads forever or silently rotate production accounts. Operators may remain on an older release while completing the work, with its ordinary support limits.
...
| Bearer credential and additional catalog adapters | Kubernetes/identity and respective catalog maintainers | Separate implementation plans before advertising their capabilities; existing private outputs remain withheld from strict ordinary tenant routes. |
