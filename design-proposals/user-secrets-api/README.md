...
- Deliver credentials repeatedly only to destinations the requester is authorized to administer, with an explicit custody boundary and observable delivery state.
...
| Coverage | Phase 1: generated-credential API, proposed PostgreSQL/MariaDB and COSI adapters, delivery into the tenant's managed Kubernetes cluster, and the classification of that cluster's administrative access. Phase 2: private inputs, initially for site-to-site VPN. Later application groups remain proposals for review. | Each included adapter must prove its own authentication and disclosure guarantees; reviewers may propose a different application grouping. |
...
| `GET/LIST/WATCH credentialoperations` | Authorized request and per-account outcomes. |
...
| `GET/LIST/WATCH credentialbindings` | Binding metadata and delivery state, including `deliveredVersion` and `adoptedVersion`. No bytes. |
...
| Local `admin` / `super-admin` | Local apps | Yes | Source use plus edit authority on the destination | Tenant-facing accounts | Supported tenant accounts | Local grants; may tighten policy | Detailed local events |
...
stateDiagram-v2
    ...
    Failed --> Usable: previous version still active
    Failed --> Requested: retry under a new operation
...
**Managed Kubernetes.** The owning tenant retains administrative access to its cluster, and that access arrives by two routes this proposal classifies differently. Where `oidc.mode` is not `None`, the per-user route is an exec-only OIDC kubeconfig carrying no token and no private key, with cluster-admin granted by a ClusterRoleBinding keyed on the user's identity claim and pruned when that user is withdrawn. It is a repeatable public projection, it needs no collection entitlement, its revocation point is the listed user plus the identity provider, and it conforms as it stands; phase 1 classifies it and adds nothing to it. The static `super-admin` kubeconfig is break-glass, as the chart itself documents it, and it is a client certificate, which Kubernetes cannot revoke individually. Two retirement mechanisms follow, and they are not equivalent. Rotating the cluster's client CA does stop the old certificate authenticating, but it has to reissue the platform's own certificates in the same operation, because remote Flux apply for every addon this chart ships and the in-cluster CSI, cloud-controller and autoscaler deployments all authenticate with the internal-endpoint key of that same Secret. Issuing the tenant-facing credential through a Kamaji `KubeconfigGenerator` instead — a CRD the platform already installs and no chart currently creates — puts it under a named identity and group whose authorization can be withdrawn by deleting its binding; that is far cheaper, but it withdraws authorization rather than rejecting authentication, and this proposal requires the evidence to tell those apart. Which mechanism the adapter takes, and in which phase, is a scope question for the API and code owners, recorded in [Open questions](#open-questions); until it is settled the static kubeconfig stays explicitly `Legacy`, and a `strictOnly` installation declines a cluster whose only administrative route is that certificate. Converting database credentials does not remove cluster ownership rights, and platform delivery access must remain usable through tenant credential replacement. The mechanism and its evidence are in [details.md](./details.md#engine-execution-and-bootstrap).

### Coverage

The proposed phase 1 rows are PostgreSQL and MariaDB managed users, COSI bucket key pairs, delivery into a namespace of the tenant's managed Kubernetes cluster, and the classification of that cluster's administrative access, whose OIDC route already conforms and needs no conversion. Phase 2 introduces write-only tenant-supplied secrets, with site-to-site VPN as its proposed first consumer, and proposes converting RabbitMQ, MongoDB, Redis/Valkey, and ClickHouse users. Kafka users and topic rights are proposed for phase 3, since the current chart has no user or SASL surface at all. NATS, OpenSearch, Qdrant, managed VPN service accounts, Harbor, and Grafana follow per adapter once nested values, derived URIs, internal users, and real rotation are covered. These application groups are an initial proposal: reviewers are invited to suggest a different distribution based on priorities and adapter dependencies.

Public CA, endpoints, and the OIDC exec-only kubeconfig stay repeatable public projections; the last of those is the tenant's conforming administrative route wherever OIDC is enabled. The static break-glass kubeconfig and other bearer access tokens require one-time collection and verified regeneration/revocation through their own adapter, whose retirement mechanism is an open scope question rather than a settled phase 1 row. Platform-internal tokens remain internal. TLS and CA private keys, Talos secrets, system backup credentials, and restic decryption keys stay internal with their current owners, and password rotation must never erase a decryption key a backup still needs. VM cloud-init and SSH private input are deferred while userData round-trips through the application spec.
...
Export lag alerts and never blocks by itself. Admission fails closed with `503 AuditUnavailable` only when the local journal cannot accept a write or its buffer reaches the reserved threshold; in that case supply and collection are refused first while revocation and delivery to bound destinations keep a reserved share. That threshold is evaluated per tenant, and non-mutating discovery and denial events are shed before it is reached, so one tenant's read traffic cannot suspend another tenant's disclosure operations. Once an engine mutation has started, recording its outcome takes priority over new work. Schema, retention, and thresholds are in [details.md](./details.md#audit-service-history-and-export).
...
A tenant provisioning through GitOps with its ServiceAccount keeps working without a human: declaring accounts requests generation, a binding delivers them into a namespace of the tenant's cluster, and only human collection needs an administrator or an account grant. After the collection window the tenant-facing routes offer regeneration rather than a second collection, and for GitOps and headless provisioning, where nobody collected in the first place, that is the common case. That bounds what those routes hand out; it does not make the material unreachable. An administrator holding source use plus edit authority on a destination can bind a delivery and read the delivered copy there — a route [Security](#security) already assigns to the tenant's own runtime, and one that is recorded as a delivery event. Recovering the stored bytes from the management cluster is not an operation of this API at all: its administrators are inside the trust boundary, and this proposal neither restricts nor audits their raw access. OIDC exec-only kubeconfigs remain repeatedly downloadable and stay the tenant's ordinary administrative route; once the static break-glass adapter is supported, an authorized owner collects that kubeconfig once and saves it, or requests regeneration after losing it. Old clients get metadata or a clear unsupported error on converted protected routes, never compatibility plaintext. Terraform providers move from password outputs to credential references and bindings.

## Upgrade and rollback compatibility

Existing-installation handoff has two ordered migration stages, A (prepare) and B (handoff); these are separate from the feature phases in [Rollout](#rollout) and apply per converted adapter. In A the API, records, audit pipeline, admission guards, and migrated clients are installed; charts stay the sole writers and pin existing credential Secrets with `helm.sh/resource-policy: keep`, and a checkpoint verifies the stored Helm revision carries it. In B charts stop rendering material-bearing Secrets, the coordinator confirms the Secret was orphaned rather than pruned and that no rendered manifest still carries it, then the controller takes the `data` field manager through a recorded server-side apply with forced conflicts, the same transfer the platform's existing migrations use for kept objects, and adopts the bytes without changing a password. `keep` gates pruning only and Helm never releases `managedFields` on its own, so the transfer is performed, not awaited. Generated bundles then leave `ApplicationDefinition` exposure and per-release raw roles, imported versions are marked `PreviouslyExposed` with collection unavailable, and an authorized regeneration creates the first strict version. A direct pre-A to B upgrade is rejected, and an unexpected deletion fails the migration with preserved material rather than triggering generation.

Strict is the default for new installations for every converted adapter; unconverted services install as reported `Legacy` unless the deployment sets `strictOnly`. Existing installations stay `Legacy` through A and B, and the deadline is per adapter: from the minor release after the one that ships a service's adapter, a tenant still holding legacy credentials in that service is recorded and reported, and creating or updating an application of that service kind in that tenant is refused until it regenerates, while its running workloads keep working and every other tenant's upgrade proceeds. The platform has no per-tenant version dimension to hang that on — `cozystack-version` is a single ConfigMap and the migration runner aborts the whole sequence on the first failing step without advancing it — so the deadline is enforced against the per-account credential records, which already carry a tenant binding, and it must degrade per tenant rather than fleet-wide. Future strict material never enters Helm history; existing revisions, etcd snapshots, Terraform state, and recipient copies are historical custody, restricted and replaced by regeneration before a strict claim.
...
Not every credential traverses all four paths today; they are exposure classes across the catalog, and closing them needs the API, charts, roles, admission, clients, and operators together. The trust boundary is stated rather than wished away: the credential controller, native issuers, nodes, administrators of the management cluster, and whoever controls a consuming workload are inside it; tenants, their other members, and their ancestors are outside it at the management-cluster custody points. A tenant's own managed cluster is that tenant's runtime: whoever the tenant lets read a delivered copy there holds the credential, and the boundary claims no authority over that RBAC. Supplier input is never a general credential oracle, because source use is checked before a privileged component contacts an endpoint and destinations are bound to UIDs and purposes. Details in [details.md](./details.md#security).
...
4. **Migration deadline per adapter.** From the minor release following an adapter's release, a tenant with unfinished legacy credentials in that service is recorded and reported, and creating or updating an application of that service kind in that tenant is refused until it regenerates; its running workloads keep working and the platform upgrade proceeds for everyone else. Services without an adapter stay reported as `Legacy`. This rule applies independently of the feature phase.
5. **Further coverage.** Kafka users and ACLs are proposed for phase 3. The static break-glass kubeconfig and other bearer tokens follow once a retirement mechanism is chosen and proven; the OIDC administrative route needs no such step. Additional services, external stores, VM and remote delivery each close their listed dependencies first.
...
- COSI replacement and revocation support in the pinned driver; absence blocks completion of the first wave as currently proposed, and the resolution is the row redistribution the phase rules already allow.
...
- Retirement of the static `super-admin` break-glass kubeconfig, and therefore its phase. The OIDC administrative route already conforms and is classified in phase 1, so what is left is a choice between two mechanisms, for the API and code owners. Client-CA rotation rejects the old certificate's authentication but must reissue the platform's own certificates with it. A Kamaji `KubeconfigGenerator`, whose CRD the platform already installs and which no chart creates today, issues the credential under a chosen identity and group that can be de-authorized by deleting a binding, which is cheaper but is authorization withdrawal rather than authentication rejection. The group the current `super-admin` certificate actually carries should be established first, since it decides whether de-authorization is available for it at all. What `strictOnly` does with an `oidc.mode: None` cluster until one of these ships is part of the same decision.
- Where the new kinds are served. `core.cozystack.io` serves only `v1alpha1` today and the aggregated server keys storage by version, so `v1alpha2` can carry the changed write-only `TenantSecret` alone while the `Credential*` kinds are born in `v1alpha1` — or the group can move together, which needs a stated `v1alpha2` contract and conversion for `tenantnamespaces`, `tenantmodules`, `options`, and `taps` as well. This proposal states no preference and asks the API owners to settle it.
