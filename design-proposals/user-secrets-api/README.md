...
A deployment whose security policy requires generated managed-service passwords, one-time disclosure, role-based access, and attributable audit needs those properties in the API. Today, hiding a password in the dashboard leaves other ways to read it, and replacing a Kubernetes Secret does not establish whether the service accepts the replacement. This proposal introduces metadata-only credential discovery, an explicit one-time collection operation, private input management, and regeneration with observable engine outcomes.

The platform retains protected material for service operation and authorized workload delivery. One-time disclosure limits collection through tenant-facing interfaces; it does not claim that the platform cannot recover a password or that a recipient forgets it. The first delivery includes PostgreSQL and MariaDB user passwords, an operator-issued S3 credential bundle, a private external replication credential workflow, and delivery of a generated credential into the tenant's own managed Kubernetes cluster. Each integration must pass its own acceptance tests before advertising support. The proposal also defines the remaining service coverage, migration from repeatable reads, and audit export without requiring an external secret manager.

## Scope and related proposals

**Supersedes [cozystack/community#72](https://github.com/cozystack/community/pull/72).** This proposal takes responsibility for credential ownership, rotation execution, bootstrap, privileged maintenance, concurrency, restore, and ownership migration. It builds on @scooby87's rotation analysis and the failure cases in [the original discussion](https://github.com/cozystack/community/issues/71). The supersession is a scope decision, not a completed GitHub action: when this proposal enters review, its author asks the maintainers to link it from the rotation pull request and close that pull request as superseded, keeping its discussion; until then both remain open and this text is the record of the intended replacement.

- [SecretRef](https://github.com/cozystack/community/pull/37) owns application reference syntax and per-engine reference conversion. This proposal supplies its private write path, supplier visibility, use authorization, ownership, and disclosure contract. It does not introduce another spelling of an application reference field. Its indefinite inline compatibility cannot apply to the strict policy defined here.
- [PostgreSQL/MariaDB password cleanup](https://github.com/cozystack/cozystack/pull/4078) remains the dependency for removing their inline password fields. Removing SQL literals alone does not remove chart-rendered Secrets from release history; that additional change belongs here.
- [API v1](https://github.com/cozystack/community/pull/73) must incorporate the reference and removed-field changes in its schema transition. An older client must not silently discard a reference and cause generation.
- [Unified TLS/PKI](../unified-tls-pki/README.md) retains certificate issuance and trust management. This proposal classifies public trust and private key material and restricts disclosure. Kubernetes application and identity maintainers retain responsibility for certificate-bearing kubeconfigs and service-account tokens; TLS issuance does not implicitly cover token revocation.
- [Tenant site connectivity](../tenant-site-connectivity/README.md) consumes private peer credentials. [ComputePlane](../compute-plane/README.md) remains a future consumer of authorized cross-cluster delivery.
- Vault, External Secrets Operator, and external issuers may supply material or receive an authorized export through future adapters. The initial implementation provides local storage and delivery. It must represent external authority and reject conflicting writers, so later integration does not require changing credential identity or weakening disclosure.
...
The source baseline is [cozystack at 67b4e23ca](https://github.com/cozystack/cozystack/commit/67b4e23ca23cb15083bc3c14985bb2e326112be4). Pending proposals and pull requests above are dependencies, not assumed shipped behavior. Runtime assertions below describe that baseline; later implementation must pin the operator and Helm versions it tests.

| Component | Relevant behavior |
|---|---|
| [Application registry](https://github.com/cozystack/cozystack/blob/67b4e23ca23cb15083bc3c14985bb2e326112be4/pkg/registry/apps/application/rest.go) | Applications are virtual views of HelmReleases. Their specs become Helm values; filtering underscore-prefixed keys does not redact passwords. There is no application-object UID suitable for an owner reference. |
| [TenantSecret registry](https://github.com/cozystack/cozystack/blob/67b4e23ca23cb15083bc3c14985bb2e326112be4/pkg/registry/core/tenantsecret/rest.go) | Projects whole Secret data. CRUD exists, but tenant write grants are absent. Writes stamp the tenant-resource marker. Update calls `UpdatedObject` with a nil old object; real PATCH behavior needs repair and integration tests before enabling writes. |
| [Lineage admission](https://github.com/cozystack/cozystack/blob/67b4e23ca23cb15083bc3c14985bb2e326112be4/internal/lineagecontrollerwebhook/webhook.go) and [controller](https://github.com/cozystack/cozystack/blob/67b4e23ca23cb15083bc3c14985bb2e326112be4/internal/lineagecontrollerwebhook/controller.go) | Admission derives projection from application provenance and leaves ownerless objects alone. The controller rebuilds its ApplicationDefinition configuration; it does not sweep and repair orphan Secret labels. Merely enabling writes would expose private inputs through the existing projection. |
| [Tenant roles](https://github.com/cozystack/cozystack/blob/67b4e23ca23cb15083bc3c14985bb2e326112be4/packages/system/cozystack-basics/templates/clusterroles.yaml) and [PostgreSQL dashboard Role](https://github.com/cozystack/cozystack/blob/67b4e23ca23cb15083bc3c14985bb2e326112be4/packages/apps/postgres/templates/dashboard-resourcemap.yaml) | Global projection grants coexist with per-release raw Secret grants. Removing either alone leaves the other. Ancestor subjects occur in generated bindings. |
| Charts and operators | `lookup` plus random generation is render-time behavior. It does not establish an observed Secret dependency. Operators accept different layouts; PostgreSQL applies SQL, while MariaDB users have native password references and root has separate bootstrap semantics. |
| Dashboard and automation | The existing secret list already transfers data before the Eye button is pressed. A Terraform resource that exports a password attribute retains it in state, so any such provider attribute is a repeatable read. Both require a changed client contract. |
| Audit | Cozystack does not provide the complete policy-and-export pipeline required here. Talos supplies a default node-local Metadata audit log ([default policy](https://github.com/siderolabs/talos/blob/main/pkg/machinery/config/types/k8s/audit_policy.go)); that is neither centralized export nor engine lifecycle evidence. Other distributions need explicit configuration. |

### The problem

A viewer can learn an inline password through application discovery. A user denied a second dashboard reveal can still retrieve the same material through a raw Secret grant. A private input written through the existing TenantSecret registry would receive an outward-projection marker. A regenerated password may reach the database before the controller records it, leaving the published value stale. Finally, an audit record saying that a backend read a Secret cannot establish which user requested it or whether a replacement became usable.

These are separate failures. Successful rotation can invalidate an old password even when historical bytes remain recoverable; erasing history and terminating existing sessions are additional operations.

## Goals

- Discover an authorized account, endpoint, database, public trust, capabilities, and lifecycle state without transferring protected material.
- Privately create, find, replace, authorize, and delete an external input; consume it in a real service without a plaintext service field.
- Generate and regenerate per account, preserve account privileges, and establish new authentication and old-credential rejection before reporting completion.
- Permit at most one tenant-facing collection of each generated version, including concurrent clients, retries, backend delegation, and component restarts.
- Deliver credentials repeatedly only to approved workload identities and destinations, with an explicit custody boundary and observable delivery state.
...
- Publish service-specific capabilities and acceptance evidence; reject unsupported operations before mutating an engine.

### Non-goals

The first release does not provide an enterprise vault, arbitrary secret sharing, scheduled rotation, dynamic database leases, delivery to VM guests, to other tenants' clusters, or to external stores, automatic restarts of arbitrary workloads, universal zero-downtime replacement, or automatic session termination. Database grants and ACLs remain engine-specific, but regeneration must preserve them.

It does not erase recipients' copies, retrospectively clean every backup, protect runtime plaintext from host/control-plane administrators, or rotate every system identity. Those exclusions do not allow protected bytes in new logs, broadly readable specs, or tenant-facing projections. Unsupported classes remain visible in the coverage table and cannot be included in a deployment-wide compliance claim.

## Design

### Summary of proposed choices

These are the choices proposed for review, not claims of maintainer approval or completed implementation. Reviewers are asked to approve the behavioral contract, the invariants, and the existence and purpose of the public surfaces. API names, wire shapes, HTTP mappings, numeric limits, and storage topology are provisional defaults; they are marked as such where they appear and collected in [Operating limits](#operating-limits), and changing them must preserve the failure semantics defined here.

| Question | Proposed answer | Rationale and detail |
|---|---|---|
| Ownership | A credential controller owns platform-generated bytes outside Helm. Native issuers keep ownership of their outputs; the controller owns lifecycle records and explicitly required delivery copies. | A template cannot keep rendered bytes out of its stored manifest. See [storage](#identity-storage-and-writers) and [handoff](#ownership-handoff). |
| One-time disclosure | One collection entitlement per generated account version, shared across all authorized recipients, exercisable within a policy-defined window after activation (default 24 hours). Discovery never contains protected fields. | Delayed collection is how creation-time disclosure is completed for asynchronous issuers and headless creation; the window is a proposed policy default, not a hidden timer. See [collection](#one-time-collection). |
| Lost or concurrent collection | Consume durably before sending bytes. One concurrent winner; a lost response after consumption requires regeneration. No payload replay or acknowledgment window. | At-most-once release is attainable; exactly-once human receipt is not. |
| Read paths | Remove protected material from app representations, legacy projections, raw dashboard grants, and future Helm renders. Classify and restrict historical copies separately. | Every effective route must respect the policy. See [Security](#security). |
| Rights and hierarchy | Separate discovery, supply, use, collection, regeneration, and delegation. Authority to declare an application's accounts carries the authority to request their initial generation and to bind them to destinations that identity manages; collection is a separate grant. No implicit ancestor credential authority. Organization policy caps all grants. | Provisioning must keep working for administrators and GitOps automation without handing them the bytes. A guessed Secret name does not authorize disclosure or use. See [authorization](#authorization-and-delegation). |
| Audit | Durable lifecycle evidence plus Metadata-level API audit; export structured events through ordinary log collectors and provide authorized service-history subscriptions. Sensitive operations fail closed only when local durable evidence cannot be recorded; export lag alerts but does not block. | API access records alone cannot prove engine activation or identify a user behind a backend; a remote SIEM outage must not stop containment or running workloads. |
| Coverage | First wave: PostgreSQL/MariaDB users, COSI-backed bucket keys, private external PostgreSQL replication authentication, and delivery into a namespace of the tenant's managed Kubernetes cluster. Later waves and unsupported classes are listed explicitly. | Covers scalar, operator-issued, supplied, and delivered material without pretending every engine has identical capabilities. |
| Managed Kubernetes access | Under strict policy a Kubernetes application must declare a tenant login path (`oidc.mode` `System`, or `CustomConfig` with a bound user); `None` is rejected at render. The admin kubeconfig is bearer material for platform delivery and audited break-glass, not tenant collection. | A cluster nobody can log into is unusable, not secure. See [engine execution](#engine-execution-and-bootstrap). |
| Compatibility | Strict is the default policy for new installations for every converted adapter; unconverted services remain installable in reported `Legacy` mode unless the deployment sets `strictOnly`. Existing installations get a finite migration window per converted adapter, followed by explicit regeneration before strict attestation. | Previously exposed bytes cannot become undisclosed by changing a label; a deadline cannot precede the adapter it depends on. See [compatibility](#upgrade-and-rollback-compatibility). |

### Identity, storage, and writers

Add a versioned API in `core.cozystack.io`, served by the aggregated API server. Public `Credential` resources contain metadata and lifecycle state; public `CredentialOperation` resources contain immutable requests and safe outcomes. Private input management is a new `TenantSecret` version. The initial shapes below use `v1alpha2` to distinguish the changed TenantSecret contract; this is a proposed API version, not an existing endpoint.

Persist an internal `CredentialRecord` CR per account and a `CredentialInputRecord` per supplier input. They live in a platform-controlled namespace, have real UIDs, and are inaccessible through tenant raw APIs. Bind a record to the tenant namespace UID, backing HelmRelease UID, and a server-assigned account incarnation. Do not owner-reference a virtual application. Removing and later re-adding an engine username creates a new account incarnation; cosmetic display-name changes do not. Kubernetes names are derived from opaque IDs, never lossy username normalization.
...
The initial live input case is PostgreSQL logical subscription to an external publisher: the supplier provides a coherent username/password bundle, and the consumer uses separately declared endpoint, database, and trust data. A restricted applier constructs connection configuration at runtime; any credential retained in the engine catalog is part of declared runtime custody and must be inaccessible to ordinary tenant discovery. Discovery reports local configuration adoption and remote authentication separately. Missing, empty, malformed, unauthorized, or remotely rejected input produces distinct safe conditions without generating a substitute. External physical replication, peer VPN secrets, SMTP, and VM bootstrap follow only when their consumer adapters exist; a generic reference alone does not claim those integrations work.
...
Local tenant administrators are trusted to delegate local account access. Users with account-management authority but without collection authority may request regeneration without acquiring any right to the previous or new payload. No tenant tier includes internal root, superuser, backup, CA private-key, or platform identity material. A support grant must be time-limited, explicitly delegated in the child scope, and audited; it still cannot reopen a spent entitlement. A fully privileged platform administrator remains outside the tenant confidentiality boundary.

The proposed portal mapping is explicit: Viewer and Project Viewer map to `view`; Member to `use`; Engineer adds supply, lifecycle, and delivery-binding authority only on assigned applications; Operator adds regeneration, revocation, and detailed history on assigned accounts; Admin maps to local `admin`. Engineer and Operator collection requires a separate account grant. These are target integration semantics, not an assertion about existing portal role bindings. Combined grants are evaluated together; merely selecting a Viewer screen cannot subtract authority independently held elsewhere. Policy can impose a subject-level disclosure prohibition that caps all those grants.

A portal backend forwards a validated end-user identity through restricted impersonation or an audience-bound delegation token. It cannot present an arbitrary `initiator` field. The API checks the intersection of the initiator's rights, the backend's delegation scope, and policy; it records both actors. No tenant-wide credential ServiceAccount is given to the portal. The upstream collection commitment spends the entitlement, including when the portal-to-browser hop fails; the backend cannot cache or retry the value for later requests.
...
**Managed Kubernetes.** Strict policy needs a tenant login path that does not depend on the admin kubeconfig, and the chart resolves the valid combinations at render time instead of leaving a cluster without access. `oidc.mode: System` requires platform OIDC to be enabled and yields the exec-only kubeconfig that is already public. `CustomConfig` requires at least one entry in `spec.oidc.users` bound to `cluster-admin`, or the tenant's own authentication configuration to grant an administrator; otherwise render fails with a message naming the missing binding. `None` fails under strict policy with a message pointing at the two supported modes. The Kamaji-issued admin kubeconfig is bearer material owned by the Kubernetes application adapter: the platform uses it for delivery into the cluster and platform administrators use it in the audited break-glass procedure; it is not tenant-collectible under strict policy and is not regenerated on tenant request in the first wave. Existing clusters with `None` stay `Legacy` until their owner sets a login path; the upgrade pre-check lists them.
...
The table describes intended rollout, not existing capabilities. Every enabled row uses the common collection, rights, audit, and local-delivery contract. An absent adapter reports its capabilities as false and rejects a mutation before staging live material. The first wave is complete only when **all** its rows pass; dropping the bucket or private-input case requires a scope revision.

| Service/account | Wave and generation authority | Replacement / revocation evidence | Disruption and remaining dependency |
|---|---|---|---|
...
| External PostgreSQL logical-subscription login | 1; supplier/external publisher | Private bundle accepted, consumer configured, remote authentication verified; source change and remote rejection visible separately | Local adoption does not revoke the publisher's old credential. Database/reference adapter owns private runtime configuration. |
...
| NATS, OpenSearch, Qdrant, VPN, Harbor, Grafana | Later, per adapter | Explicit capability and audience per account; URI/config/hash/key variants treated as protected | No strict claim until nested values, derived material, internal users, and actual rotation/revocation are covered. Qdrant's two privilege levels do not prove same-identity overlap. |
...
| Managed Kubernetes admin kubeconfig, long-lived service-account token | Bearer; not tenant-collectible under strict policy. The admin kubeconfig is the platform's delivery channel and the break-glass credential | Tenant access is through the declared login path (`System` or `CustomConfig`), verified at render; self-service regeneration of bearer material follows once Kamaji reissue is proven | A strict Kubernetes application with `oidc.mode: None` is rejected at render rather than created without access. Certificate download does not make revocation equivalent to password rotation; CA rotation may affect other identities. Break-glass use is an audited platform-admin procedure outside tenant collection. |
...
```json
{
  ...
  "executor": {"subject": "serviceaccount:portal"},
...
CLI and portal follow the same create → poll metadata → explicit collect sequence. A regeneration request reports per-account progress, and a user can distinguish engine activation from consumer adoption. A viewer receives connection metadata and sanitized history without a generation, collection, or delivery route. Private inputs appear in their supplier's inventory with dependencies and safe validation status, plus replace and retire actions; there is no Eye button for readback.

A tenant that provisions applications through GitOps with its tenant ServiceAccount keeps working without a human in the loop: declaring accounts requests their generation, and a binding delivers them into a namespace of the tenant's Kubernetes cluster; only human collection needs an administrator or an explicit account grant. A Kubernetes application created under strict policy must declare its login path, and the render error says which mode to set.

Old clients receive metadata or a clear unsupported/deprecated error for protected routes, never compatibility plaintext. Terraform providers migrate from password outputs to credential references and bindings. Operators get documented recovery commands and version-specific adapter diagnostics without needing unrelated root credentials in their terminal output.

## Upgrade and rollback compatibility

### Ownership handoff

Use two named platform release phases, **A (prepare)** and **B (handoff)**. Exact release numbers are assigned when implementations are ready; the supported transition is fixed here. The upgrade coordinator inventories the installed chart/operator versions, release history, actual Secret UIDs, field managers, raw grants, projection selectors, and runtime consumers. A missing Secret does not classify an installation as new: freshness requires a newly registered application incarnation with no migration or restore lineage.
...
Protect live Secrets, engine authentication catalogs/verifiers, mounted copies, and recovery backups through the installation's approved encryption-at-rest and backup controls. Strict activation verifies those controls on each supported distribution; it does not assume Talos defaults elsewhere. External KMS is optional. The central credential controller, native issuer/operator, hosting nodes, cluster administrators, and authorized workload operators form the stated custody boundary. Namespace-scoping a shared identity does not reduce that identity's total authority.
...
| Two tabs, two API replicas, or portal retries collect | One conditional commitment; other attempts fail. No payload replay, including an ambiguous successful commit. |
...
| Strict Kubernetes application without a login path | Render rejects `oidc.mode: None`, `System` with platform OIDC disabled, and `CustomConfig` without a bound administrator, each with a safe message naming the fix; existing clusters stay `Legacy` until a login path is set. |

## Testing

This is a design acceptance plan. No live engine, migration, browser, or SIEM test is claimed as executed by this proposal. API-only tests may require a real API server, admission, RBAC, client, and collector; mocks do not replace those boundaries. Live tests use pinned engines/operators and actual authentication.
...
| One-time live flow | For each first-wave generated service: create through UI/API, obtain a usable bundle once, deny repeat through every effective route, regenerate, prove new authentication and old rejection, then allow only the new version. Repeat through delegated portal users and headless creation. |
...
| Roles and delegation | Viewer/Project Viewer, all target portal mappings and combined roles, explicit grants, local/ancestor administrators, ServiceAccounts, forged initiator claims, revoked membership, subscriptions, and tenant transfer. Check actual direct and delegated authority and the five-second bound. |
| Private external authentication | Live PostgreSQL logical subscriber and external publisher: private supply, delayed/malformed input, real authenticated consumption, source replacement, remote rejection, and secret-free public config/diagnostics. Verify engine-catalog custody restrictions. |
...
| Leak diagnostics | Inject unique synthetic material into valid and malformed supply, failed engine authentication, SQL failures, collection, proxies, portal retries, and regeneration. Scan API/engine/operator/applier logs, Events, status, traces, history, alerts, and exported sink records. |
...
1. **Proposal review:** API owners `@lllamnyp` and `@kvaps` review the eight choices, scope, and authorization boundary. Engine, storage, backup, client, and security maintainers review their evidence obligations. This draft does not substitute for their approval.
2. **Foundation / A:** implement records, private TenantSecret version, old-version filters, closed schemas, policy, metadata clients, journal/export, and admission guards. Prepare existing-release checkpoints. Enable input writes only after end-to-end privacy tests pass. Legacy accounts remain visibly legacy.
3. **First-wave adapters / B:** implement PostgreSQL, MariaDB, the pinned COSI driver, external logical-subscription input, and delivery into tenant-owned managed Kubernetes clusters; complete the first-wave live suites, restore, and actual ownership handoff. Strict behavior is default for fresh conforming installs; existing tenants complete explicit migration and regeneration.
4. **Migration deadline:** for each converted adapter, the minor release following the one that shipped it blocks upgrades with unfinished legacy credentials in that service; services without an adapter stay reported as `Legacy`. Strict attestation requires effective-route tests, current generated versions, configured retention, functioning SIEM export/subscriptions, and compatible clients. It is an adapter- and deployment-scoped statement, not a blanket catalog claim.
5. **Second wave:** RabbitMQ, MongoDB, Redis/Valkey, and ClickHouse each land with their own adapter, privacy conversion, and live acceptance. No release may advertise parity based only on a common endpoint.
6. **Further coverage:** Kafka user/ACL support, bearer credentials, additional services, external stores, and remote/VM delivery each close the listed dependencies before adding capabilities. TLS issuance remains with its own proposal.

Proposal acceptance means agreement on this contract and its choices. Feature acceptance requires the evidence above. Implementation changes that invalidate a guarantee or first-wave row require revising this proposal, with a decision record once the changed choice is settled.

## Open questions

The eight architectural choices above are answered. The remaining items are implementation evidence and release assignments with defined blocking consequences; none is permission to ship weaker behavior silently.

| Remaining item | Owner | Required evidence and deadline |
|---|---|---|
| Exact A/B release numbers and compatible component versions | Release and API maintainers | Publish transition matrix before A release freeze; B cannot ship without successful pinned handoff tests. |
| MariaDB missing-input, native reconcile, and standalone revoke behavior | MariaDB maintainer | Live empty-auth, stale-writer, recreate, and revoke tests before B; failure blocks that first-wave row. |
| COSI replacement/revocation API and issuer entropy | Storage maintainer | Pinned driver implementation and real new/old authentication evidence before B; absent support blocks completion of wave 1. |
| PostgreSQL restore maintenance authority and external subscription diagnostics | Database/backup maintainers | Source-unavailable restore and private runtime-configuration tests before B; no assumption that a current source password matches restored data. |
| Actual portal role and identity transport conversion | Portal/API maintainers | Effective-principal tests against the target mapping before strict portal enablement. |
| Runtime capability isolation and audit capacity on supported distributions | Platform/security maintainers | Admission/exec/VM access inventory, collector sink tests, and capacity measurements before strict activation on each distribution. |
| Bearer credential self-service (admin kubeconfig reissue) and additional catalog adapters | Kubernetes/identity and respective catalog maintainers | Separate implementation plans before advertising their capabilities; tenant access to strict clusters is through the declared login path meanwhile. |

The public API names and numeric limits are proposed defaults subject to review. Changing them must preserve the defined failure semantics, finite migration boundary, and minimum first-wave acceptance set.

## Alternatives considered

**Keep generation in Helm or centralize it in a template helper.** A helper can improve formatting and policy reuse, but rendered material still enters release history and `lookup` is still not a lifecycle dependency. Referencing controller/operator-owned material addresses both limits. The [rotation proposal](https://github.com/cozystack/community/pull/72) motivates ownership transfer, but its assumption that removed templates automatically relinquish fields is replaced here by an inspected, tested handoff.

**Return values only from create/regenerate, or permit replay until acknowledgment.** Inline results do not accommodate delayed operator issuance and unattended creation cleanly. A replay window or per-person entitlement permits more than one platform release of the same bytes. Separate collection with consumption before response gives a precise loss behavior at the cost of requiring regeneration after an uncertain response.

**Keep raw reads for ServiceAccounts and hide the dashboard button.** The same principal can use another client, and a person controlling a workload can exercise its runtime authority. Explicit delivery bindings, policy checks, and the stated workload trust boundary are enforceable; client type and UI state are not.

**Enable existing TenantSecret write verbs without changing projection.** Existing writes stamp the outward marker and return data. A private supplier workflow therefore needs versioned serialization, policy, identity, and filtering before granting verbs. A wholly separate input API is viable, but would leave two tenant secret management surfaces to explain and migrate; versioning the existing one preserves its purpose while fixing its contract.

**Use a generic subresource on every virtual app with no durable records.** A convenience app-level route may be added later, but it cannot provide stable account incarnation, consumption state, replay ordering, and recovery by itself. Public credentials backed by durable records give those operations an identity independent of virtual application UIDs and chart naming conventions.

**SQL Jobs for every database, including MariaDB users without password references.** As explored in [the rotation discussion](https://github.com/cozystack/community/issues/71), removing a native password reference can expose a bootstrap/recreation gap and create two writers. This proposal keeps MariaDB's native writer and uses Jobs where needed, with independent verification for both. Standing privileged connections simplify dispatch but extend privileged session lifetime; bounded Jobs and explicit fencing are the initial choice.

**Rotate maintenance root in the same tenant operation.** That changes the authority required to recover the operation itself. The initial service-user lifecycle excludes internal maintenance accounts and explicitly withholds them; a later engine-specific handoff must prove recovery at every interruption point before internal rotation is enabled.

**Use only API audit, Kubernetes Events, or best-effort logs.** API audit supplies access evidence but not engine outcomes or service-history subscriptions. Short-lived Events and unacknowledged logs cannot provide the durable consumption record or recoverable export required here. The journal/outbox design costs storage and can reject new work during an audit outage; that availability tradeoff is explicit.

**Inherit parent/use-tier collection and keep repeatable reads as the permanent default.** That preserves current convenience but cannot satisfy account isolation or strict one-time disclosure. Separate grants and a finite migration window make the breaking change visible. An indefinite compatibility profile would leave the core security problem unresolved.

**Require Vault/KMS or promise that no component can recover plaintext.** External custody can improve a deployment's trust boundary, but does not remove the need to deliver bytes to password-consuming engines. The local baseline explicitly trusts runtime storage and platform operators. External source/export adapters remain possible without treating their authority as platform-owned or allowing overwrite loops.

**Ship only PostgreSQL/MariaDB and call the API complete.** That would leave operator-issued multi-field credentials and private external input untested. The first-wave S3 and replication workflows are intentional acceptance obligations. Broader catalog parity, automatic anti-rollback after arbitrary control-plane restore, and remote delivery remain explicit later work.
