...
Add a versioned API in `core.cozystack.io`, served by the aggregated API server. Public `Credential` resources contain metadata and lifecycle state; public `CredentialOperation` resources contain immutable requests and safe outcomes. Private input management is a new `TenantSecret` version introduced in feature phase 2. The shapes below describe the target API across phases and use `v1alpha2` to distinguish the changed TenantSecret contract; this is a proposed API version, not an existing endpoint. The shapes below show the new kinds in `v1alpha2` because the write-only `TenantSecret` contract is incompatible with what `v1alpha1` serves and so needs a version of its own. That incompatibility forces nothing else: the aggregated server keys storage by version, so `v1alpha2` could carry the changed `TenantSecret` alone while the `Credential*` kinds are born in `v1alpha1`, or the group could move together at the cost of a stated `v1alpha2` contract and conversion for the four kinds already served. This proposal does not settle that; it is listed in the README's open questions for the API owners. The public API provides authorized abstractions over persistent internal objects; aggregation does not make their state ephemeral.
...
The credential record holds policy revision, active and candidate version IDs, operation ordering, collection state, and a bounded transactional audit outbox. It contains no material or material-derived fingerprints. Protected Secrets store immutable candidate/version bundles, linked to record UID and random version ID. Canonical generated versions live with the platform records; native adapter copies and SecretRef inputs live in the consumer's tenant namespace. Cross-namespace relationships use explicit UID mappings and controller finalization, never invalid cross-namespace owner references. Finalizers are placed only on platform-side records, never on the tenant namespace, its Secrets, or the application release, so an unavailable controller cannot block tenant deletion. The controller creates a candidate once before any engine mutation; recovery reuses it. Losing material while its record survives is `MaterialMissing`, not evidence of a fresh install.
...
| Local `admin` / `super-admin` | Local tenant apps | Yes | Yes, with source use plus edit authority on the destination | Yes, tenant-facing accounts only | Yes, supported tenant accounts | Local grants; may tighten policy | Detailed local tenant events |
...
Administrative access arrives by two routes with different classifications. Where `oidc.mode` is not `None`, the per-user route is the exec-only OIDC kubeconfig: no token, no private key, cluster-admin carried by a ClusterRoleBinding keyed on the identity claim and pruned when the listed user is withdrawn. It is a repeatable public projection with no collection entitlement, its revocation point is the listed user plus the identity provider, and it conforms as it stands; phase 1 classifies it and adds nothing to it.

The static `super-admin` kubeconfig is a break-glass client certificate in `system:masters`. That group bypasses RBAC, so authorization cannot be withdrawn from it, and Kubernetes has no per-certificate revocation, so rotating the cluster's client CA is the only mechanism that retires it — and the rotation must reissue the platform's own certificates in the same operation, because remote Flux apply and the in-cluster platform components authenticate with certificates from that CA. An admin kubeconfig carrying a private key or token is otherwise a tenant-facing credential bundle: one collection per generated version, regeneration after loss or expiry of collection, and verified loss of access with the old credential before replacement completes. Whether that adapter lands in phase 1 is a scope question for the API and code owners, recorded in the README's open questions. Until it is settled the static kubeconfig remains explicitly `Legacy` and is excluded from strict claims, and a `strictOnly` installation declines a cluster whose only administrative route is that certificate. An unrelated database adapter's migration must not remove the owner's Kubernetes access.

Kubernetes [client-certificate authentication](https://kubernetes.io/docs/reference/access-authn-authz/authentication/) does not provide individual certificate revocation; a new Kamaji issuance or deletion of the stored kubeconfig alone does not invalidate the old certificate. Any adapter for the static route must therefore retire access at the certificate authority and prove it, never claim that reissue is revocation. A successful new administrator request and verified denial with the old credential must cover the intended cluster and authority; distinguish authentication rejection from withdrawal of authorization in the reported evidence. Existing sessions are separate. Tenant-facing credentials and the platform's delivery/maintenance access have distinct lifecycle requirements: the adapter must preserve remote Flux apply and bound delivery when tenant credentials are replaced. No particular token, CA-rotation, or identity mechanism is approved by this proposal.
...
| Public CA, endpoints, OIDC exec-only kubeconfig | Existing public projections, preserved; 1 for the classification | Repeatable public discovery; no collection entitlement. For the OIDC kubeconfig: cluster-admin follows the identity claim, and withdrawing a listed user removes access | PKI and application maintainers ensure outputs contain no token, private key, or embedded password. This is the tenant's conforming administrative route wherever OIDC is enabled. |
| Static `super-admin` break-glass kubeconfig and other bearer access tokens | Phase open for review; retirement mechanism is a scope question for the API and code owners | One collection per generated version; fresh admin access with the replacement; verified loss of old access before completion, with authentication and authorization outcomes distinguished | The tenant retains cluster administration through the OIDC route above, which already conforms. `system:masters` bypasses RBAC and certificate reissue alone does not revoke, so retirement means rotating the cluster client CA and reissuing the platform's certificates with it. Stays Legacy until settled, and `strictOnly` declines a cluster whose only administrative route is that certificate. Preserve the platform's separate delivery/maintenance lifecycle; internal platform tokens remain internal. |
...
3. In B, charts stop rendering material-bearing Secrets and all other protected payloads. Keep the existing Secret UID/name and runtime references while verifying the successful Helm upgrade did not prune it. Inspect actual `managedFields` and release membership. Do not assume removing a template releases SSA ownership: Helm never does, and `keep` gates pruning only. Only after Helm no longer renders those bytes may the migration controller perform a narrowly scoped, recorded ownership transfer, taking the `data` field manager through server-side apply with forced conflicts as the platform's existing migrations already do for kept objects; unexpected writers or release membership block it.
...
New installations enable strict behavior for every converted adapter. Services without a converted adapter remain installable and are reported per application as `Legacy`, which excludes them from any compliance attestation; a deployment whose policy cannot tolerate that sets the platform option `strictOnly`, which makes unconverted secret-bearing services unavailable rather than legacy. Existing installations remain in explicitly reported `Legacy` mode through A and B. The migration deadline is per adapter: from the minor release after the one that ships a service's adapter, a tenant that still holds legacy credentials in that service is pinned and reported, and its conversion-dependent operations are refused. It is enforced against the per-account credential records, which already carry a tenant binding, and not against the stored platform version: `cozystack-version` is a single ConfigMap in `cozy-system` with no per-tenant dimension, and the migration runner walks migrations in sequence and aborts the whole run on the first non-zero step without advancing that version, so a deadline enforced there would let one tenant's unregenerated password freeze every other tenant's upgrade on that cluster. The required shape is therefore warn-pin-skip: record the offending tenant, report it, refuse only that tenant's conversion-dependent operations, and let the platform upgrade proceed. It never blocks on services that have no adapter yet, does not silently keep repeatable reads forever, and does not silently rotate production accounts. Operators may remain on an older release while completing the work, with its ordinary support limits.
...
| Static admin kubeconfig adapter cannot retire old access | No strict replacement or collection capability is claimed; preserve existing Legacy admin access until a supported adapter and migration exist. Ownership rights are never removed as a substitute for credential lifecycle support, and the platform-wide default still does not require OIDC; only a `strictOnly` installation declines a cluster whose sole administrative route is that certificate. |
...
| Tenant Kubernetes access | Phase 1: repeatable secret-free OIDC kubeconfig discovery, cluster-admin actually following the identity claim, and withdrawal of a listed user removing access. When the static-route adapter ships: authorized one-time admin bundle collection; replacement with preserved cluster-admin authority; verified loss of old access; continued platform delivery/maintenance access. Reissuing a certificate without retiring old access must not pass. Migration preserves owner access, and no new OIDC-mode requirement is introduced outside `strictOnly`. |
