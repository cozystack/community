...
| [community#25](https://github.com/cozystack/community/pull/25), [community#33](https://github.com/cozystack/community/pull/33) | Per-cluster etcd; ComputePlane as an operator-owned module | Downstream consumers of the tiering; not blocked by this proposal |
...
### 4. The versioned pool — two options and the experiment that decides
...
- **Private repositories** are handled by #23's `secretRef` threading; nothing here changes it.
...
2. **Experiment and decision.** Run the `ExternalArtifact` revision experiment; record the result in this document; choose Option A or Option B.
...
- **Option A or Option B** — resolved by the experiment in phase 2. Everything else in the design is stable across the answer; only the implementation site changes.
