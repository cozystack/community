...
- **Dry-run / recommendation** — render the dashboard and alert rules and still render the `ScaledObject`, but paused in **both** directions via KEDA's per-direction pause annotations (`autoscaling.keda.sh/paused-scale-in` **and** `autoscaling.keda.sh/paused-scale-out`, both `"true"`): the HPA stays up and keeps serving its trigger metric, but its `desiredReplicas` is frozen to the current count *for a source already at or above the quorum floor*, so behavior can be observed — read the recommendation off the read-load panel (`desired = 1 + ceil(load/target)`) rather than the frozen desired-replicas line — before actuation is enabled. The freeze is **not** absolute for a single-instance source: with `minReplicaCount` pinned to `effectiveMin ≥ 2`, the stock HPA's `currentReplicas < minReplicaCount` branch writes `/scale = effectiveMin` unconditionally and never consults the pause annotations (see the phase-1b caveat and the Enablement constraint under [Upgrade and rollback compatibility](#upgrade-and-rollback-compatibility)), so `dryRun` on an `N = 1` source bumps it straight to the floor rather than holding it — the same conscious footprint change as enabling actuation. `dryRun` is an ordinary values-schema flag a tenant may set directly, not only a phase-1 step, so this caveat applies wherever it is used. This is deliberately **not** KEDA's full `autoscaling.keda.sh/paused` annotation, which deletes the HPA and would leave no recommendation to observe.
