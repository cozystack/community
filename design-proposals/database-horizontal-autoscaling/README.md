...
- **Status:** Draft
- **PoC:** `2026-08-10` — live validation confirmed the mechanism and pinned a hard precondition: the CNPG `Cluster` `/scale` subresource must serve a non-empty `status.selector` (else the HPA rejects the target with `reason=InvalidSelector`) — a behavioural gate, not a version floor (carried by 1.28.4+, 1.29.2+, 1.30.0+ but absent from 1.29.0/1.29.1); see [PoC finding](#poc-finding-2026-08-10--cnpg-version-floor-for-hpa-actuation)
...
> **Note (2026-08-10):** a live PoC validated this design and pinned a behavioural precondition — a stock HPA can only drive the CNPG `Cluster` scale subresource when that subresource serves a non-empty `status.selector` (otherwise the HPA rejects the target with `reason=InvalidSelector`); this is a served-selector gate, not a version floor (cloudnative-pg#8996). The mechanism is unchanged; see the [PoC finding](#poc-finding-2026-08-10--cnpg-version-floor-for-hpa-actuation) below.
...
A live PoC on a dev cluster (cozystack v45, CloudNativePG 1.27.3, Kubernetes 1.34.3) validated this design and pinned one hard precondition: the actuation path works only where the CNPG `Cluster` `/scale` subresource serves a non-empty `status.selector` (without it the HPA rejects the target with `reason=InvalidSelector` before any metric branching) — a behavioural gate, not a version floor (see [§Scope](#scope-and-related-proposals) for the release matrix). This section is a record, not an open decision — the mechanism is unchanged.
...
**Resolution — no design change.** On any CNPG that serves a non-empty `/scale` `status.selector` the approved mechanism (chart conditional + KEDA `ScaledObject` + the `Σ + target` query) works byte-for-byte on stock CNPG against a real operator-maintained selector. The fix was a routine vendored-chart bump, since merged ([cozystack/cozystack#3951](https://github.com/cozystack/cozystack/pull/3951)): `packages/system/postgres-operator` now vendors CNPG 1.30.0, above the floor, so no design change and no pending dependency remain — a re-run of this same PoC on the vendored operator is the verification gate. Two alternatives were considered on the way to this record and are **rejected** as unnecessary once the selector ships: (A) KEDA plus a thin mirror shim behind a proxy object that exposes a selector; (B) a lean actuation controller writing `Cluster.spec.instances` directly. (A) adds a CRD and a controller to work around a gap that no longer exists; (B) re-acquires the hand-rolled decision loop this design spent three revisions deleting. Neither would have re-introduced rev1's ownership machinery — §3 (the chart ceding the field to the HPA) already removed that — but neither is needed.
...
**Precondition — the CNPG scale subresource must serve a selector.** The `Cluster` `/scale` subresource must serve a non-empty `status.selector` (wired via `labelSelectorPath: .status.selector`), or the Kubernetes HPA rejects the target with `ScalingActive=False, reason=InvalidSelector` before any metric-type branching and never scales (see the [PoC finding](#poc-finding-2026-08-10--cnpg-version-floor-for-hpa-actuation)). The selector shipped in cloudnative-pg#8996 and is carried by CNPG **1.28.4+, 1.29.2+, and 1.30.0+**, but is absent from **1.29.0 and 1.29.1** (cut before the 1.29-line backport) — so the gate is the selector actually being served, not a monotone `≥ 1.28.4` version bound, since 1.29.0 and 1.29.1 are numerically higher yet fail it. cozystack vendors CNPG 1.30.0 since [cozystack/cozystack#3951](https://github.com/cozystack/cozystack/pull/3951), which serves it.
...
The seed is a constant, so every reconcile renders the value it rendered last time; under **client-side** apply helm-controller patches the CNPG `Cluster` (a CRD) from the previous-versus-new rendered manifest and — under its current default, two-way merge for unstructured objects on the upgrade path every steady-state reconcile takes ([decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md); the narrow install-with-adoption recovery path is the documented exception that does read live) — does not consult live state, so the unchanged seed produces no patch and the HPA — writing the `scale` subresource — keeps sole effective control of `.spec.instances`. Client-side apply is forced per `ApplicationDefinition` (i.e. per kind — it reaches every Postgres, see [decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)) by `release.cozystack.io/helm-server-side-apply: "false"`, because the platform's helm-controller (v1.5.0) defaults to server-side apply, under which a *rendered* field is force-owned by Flux and reverted from the HPA's live value on every apply. This is what settles the ownership problem — no marker annotation on the CR, no admission webhook, and no terminal-freeze conflict handling is needed.
...
- **Scale-down pacing** — a literal `behavior.scaleDown.policies: [{type: Pods, value: 1, periodSeconds: ~600}]` in the rendered object, so at most one standby is removed per period (restoring the step-of-1 conservatism; the default HPA policy would allow removing 100% of pods in 15s). This pacing bounds only the in-range branch: the stock HPA consults these `scaleDown` policies solely inside `normalizeDesiredReplicasWithBehaviors`, which the two out-of-range branches never reach — a source above `maxReplicaCount` is cut straight to `maxReplicaCount` and a source below `minReplicaCount` is bumped straight to `minReplicaCount`, each in a single unpaced write. `periodSeconds` is a deliberate value on the order of minutes, sized against replica provisioning latency (see [Failure and edge cases](#failure-and-edge-cases)).
...
- **Dry-run / recommendation** — render the dashboard and alert rules and still render the `ScaledObject`, but paused in **both** directions via KEDA's per-direction pause annotations (`autoscaling.keda.sh/paused-scale-in` **and** `autoscaling.keda.sh/paused-scale-out`, both `"true"`): the HPA stays up and keeps serving its trigger metric, but its `desiredReplicas` is frozen to the current count *for a source already at or above the quorum floor*, so behavior can be observed — read the recommendation off the read-load panel (`desired = 1 + ceil(load/target)`) rather than the frozen desired-replicas line — before actuation is enabled. The freeze is **not** absolute for a single-instance source: with `minReplicaCount` pinned to `effectiveMin ≥ 2`, the stock HPA's `currentReplicas < minReplicaCount` branch writes `/scale = effectiveMin` unconditionally and never consults the pause annotations (see the phase-1b caveat and the Enablement constraint under [Upgrade and rollback compatibility](#upgrade-and-rollback-compatibility)), so `dryRun` on an `N = 1` source bumps it straight to the floor rather than holding it — the same conscious footprint change as enabling actuation. The freeze is equally **not** absolute for a source above the ceiling: the stock HPA's mirror `currentReplicas > maxReplicaCount` branch writes `/scale = maxReplicaCount` unconditionally and likewise never consults the pause annotations, so `dryRun` on a source whose live count exceeds `maxReplicaCount` (`autoscaling.maxReplicas` defaults to 6 — e.g. an 8-instance cluster) is cut straight to `maxReplicaCount`, shedding the excess standbys and their PVCs, and the one-per-period `scaleDown` pacing does not bound the cut because the above-max branch never reaches the rendered policies — raise `autoscaling.maxReplicas` to at least the live count before enabling `dryRun` to avoid it. `dryRun` is an ordinary values-schema flag a tenant may set directly, not only a phase-1 step, so this caveat applies wherever it is used. This is deliberately **not** KEDA's full `autoscaling.keda.sh/paused` annotation, which deletes the HPA and would leave no recommendation to observe.
  ...
  triggers:
    - type: prometheus
      metadata:
        ...
        ignoreNullValues: "false"  # empty/stale result ⇒ FailedGetExternalMetric (hold), not 0 (scale to floor)
```
...
- **Enabling autoscaling on an existing database.** Because the chart always renders `.spec.instances` — a constant seed under autoscaling, not an omitted field — there is no field-deletion hazard on the flip. The two cares that remain are that the seed match the live count at flip time and that KEDA come up warm rather than actuating from cold. The committed two-phase order (with an optional observation window between them): **(phase 1 — stand up warm and paused)** the operator reads the live `.status.instances` (= N) and sets `.Values.replicas = N` (a no-op to the running cluster), then sets `autoscaling.enabled: true` with a transition sub-flag that renders `instances: {{ .Values.replicas }}` (= N) alongside the new `ScaledObject` paused in **both** directions (`autoscaling.keda.sh/paused-scale-in`/`paused-scale-out`, `minReplicaCount` pinned to N) — the same non-actuating mode as §5's dry-run, so KEDA stands up its HPA warm and begins observing load while actuating neither way. **(phase 1b — dry-run observation, optional but recommended)** hold in this paused state and read the recommendation off the read-load dashboard panel (`desired = 1 + ceil(Σ/target)`) to confirm the metric and bounds are sane before handing over actuation; because the count is pinned to N throughout, this window is inert on the running cluster **for a multi-instance source** (N ≥ the floor). It is **not** inert for a single-instance source: the quorum floor forces `minReplicaCount` to `effectiveMin ≥ 2`, and the stock HPA controller writes `/scale = minReplicaCount` unconditionally whenever `currentReplicas < minReplicaCount` — that branch never consults the pause annotations (`kubernetes/kubernetes@v1.33.4 pkg/controller/podautoscaler/horizontal.go`), so a cluster at `N = 1` is bumped straight to `effectiveMin` the moment KEDA's HPA comes up. It is equally **not** inert for a source above the ceiling: the mirror branch writes `/scale = maxReplicaCount` unconditionally whenever `currentReplicas > maxReplicaCount` (`autoscaling.maxReplicas` defaults to 6), so an 8-instance cluster under a `maxReplicas` of 6 is cut straight to 6 the moment KEDA's HPA comes up — shedding two standbys and their PVCs during the observation window, and because the above-max branch never reaches the rendered `scaleDown` policies the one-per-period pacing does **not** bound it. Staging `.Values.replicas` to the live count cannot prevent this: the trigger is `maxReplicaCount` against the live count, not the rendered `instances`. This is legitimate — it is the same `minReplicas ≥ 2` footprint change flagged under the Enablement constraint below — but it must be a conscious step, not observation-only: stage `.Values.replicas = effectiveMin` before phase 1 so the seed already matches the floor, or accept the one-step bump to the floor as part of enabling; and if the live count exceeds `autoscaling.maxReplicas`, raise `autoscaling.maxReplicas` to at least the live count before phase 1 so KEDA's HPA does not shed standbys on cold start. No render guard catches the above-max case today — the chart fails on `effectiveMin > maxReplicas` and on `replicas <= maxSyncReplicas`, but nothing rejects or warns on `replicas > maxReplicas` (follow-up for the implementation: cozystack/cozystack#3954). **(phase 2 — hand over actuation)** clear the transition flag, so the chart renders the constant seed `max(replicas, effectiveMin)` — the same value N when `replicas` is staged at/above the floor — and unpauses the `ScaledObject`. All three stages render the same count, so the client-side patch (previous-versus-new manifest) is empty and the live count never dips. What keeps Flux from reverting the HPA's subsequent `/scale` writes is client-side apply, not a field handoff: see [decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md), which records why the platform default (server-side apply) reverts a rendered field and why the client-side two-way merge does not.
- **Steady state after migration is correct.** The seed is a constant present in every render, so helm-controller's client-side patch — computed from the previous versus the new rendered manifest, and, under its current two-way-merge default on the upgrade path ([decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md); the install-with-adoption recovery path is the documented exception that does read live), not from live state — re-asserts nothing, and the HPA-set `.spec.instances` is left untouched. The exception is anything that moves the *rendered* seed between two applies: a tenant edit to its inputs (`.Values.replicas`, `autoscaling.minReplicas`, `quorum.maxSyncReplicas`), or — less obvious — a platform chart upgrade that changes how the seed is computed (the `effectiveMin`/seed helper or a schema default), which rebases an HPA-grown `.spec.instances` on the next apply with no tenant edit at all. Either way that single reconcile patches `.spec.instances` to the new seed (never below the quorum floor and never a collapse to 1, though an HPA-grown count above the floor is still dropped back to the floor). Stage `.Values.replicas` to the live count first — or use the two-phase `transition` flow — to rebase deliberately with no transient; treat a chart bump of an autoscaling-active database the same way rather than applying it blind. A third case moves `.spec.instances` the same way but out of the seed branch: flipping an autoscaling-active cluster the HPA has grown into a staging mode (`transition` or `dryRun`) while `enabled` stays true re-renders the static `instances: {{ .Values.replicas }}`, which is **not** clamped to the floor, so it sheds the live count down to `replicas` unless `.Values.replicas` is first staged to that live count — stage it before any such flip, exactly as for the enable/disable flows.
- **Restore rebases to the seed by construction.** Restoring an autoscaling-active database — in place, or into an existing application that already has autoscaling on — deletes the CNPG `Cluster` and its PVCs outright and re-creates the `Cluster` from `bootstrap.recovery` ([cozystack/cozystack#3959](https://github.com/cozystack/cozystack/pull/3959)), so the restored cluster comes up at the rendered seed `max(replicas, effectiveMin)` (the quorum floor) rather than the count the HPA had grown to; the surviving `ScaledObject`/HPA then re-scale it back to load as the recovery catches up. This is a distinct write path from the three seed-branch rebases above — the object is physically re-created, not patched — and the staging recipe used for those does **not** apply here: the restore erases live state on purpose, so pre-staging `.Values.replicas` cannot preserve the grown count. It is a conscious rebase-to-seed on recovery, bounded exactly as the others (never below the floor, never a collapse to 1) and never a data-loss path; see [decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md).
...
- **Enablement constraint — `minReplicas ≥ 2` changes single-instance footprint.** Enabling autoscaling on a current single-instance Postgres permanently doubles instances (a second replica's PVC and DRBD volume). This is legitimate but must be a conscious enablement decision, not a surprise — and it is load-bearing, since at `Σ = 0` the formula yields `desired = 1`. This footprint change lands as soon as autoscaling is enabled, even during the paused phase-1 window: the HPA's `currentReplicas < minReplicaCount` branch bumps a single-instance cluster to `effectiveMin` immediately (see the phase-1 caveat above), so phase 1 is not a no-op observation window for a single-instance source.
...
- **Blast radius.** No cluster-wide admission webhook (a key regression of rev1 is gone). The new platform-wide surfaces are all deliberate, reviewed dependencies rather than incidental ones: KEDA claiming the `external.metrics.k8s.io` APIService, the cluster-wide RBAC KEDA needs to write CNPG `clusters/scale`, and the per-kind client-side apply this design forces for every Postgres ([decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)).
...
- vmselect unreachable or metric missing → the HPA has no metric and holds the current count (`ScalingActive=False`); the alert rules fire. No blind scaling. This rests on `ignoreNullValues: "false"` in the rendered trigger (§User-facing changes): it is what makes an empty or stale PromQL result surface as `FailedGetExternalMetric` — so the HPA holds the current count — rather than being read as `0`, which KEDA's default (`ignoreNullValues: true`) does and which would drive `desired = ceil(0/target) = 0` and scale down to the quorum floor. Without it the fail-safe is inverted: a monitoring gap becomes a scale-down instead of a hold.
...
- **PoC first — validate the single-value metric (§1) against a real HPA:** confirm the `Σ + target` query with an `AverageValue` threshold drives a KEDA-managed HPA to `1 + ceil(Σ/target)` across the `ceil` boundaries, including `Σ = 0 → 1` clamped up by `minReplicaCount`. Also confirm the load-bearing precondition the HPA rests on for **whichever engine ships**: the engine CR's `scale` subresource must serve a `status.selector` (wired via `labelSelectorPath`), or the HPA fails every target type with `ScalingActive=False, reason=InvalidSelector` before any metric branching. This is a property of the stock HPA controller against *any* scale subresource, not a CNPG quirk. For the MVP's PostgreSQL this gate is met by any CNPG release that carries the selector (1.28.4+, 1.29.2+, 1.30.0+ — but not 1.29.0/1.29.1; see [§Scope](#scope-and-related-proposals) for the full matrix and why it is a selector check rather than a `≥ 1.28.4` bound); main vendors 1.30.0 since [cozystack/cozystack#3951](https://github.com/cozystack/cozystack/pull/3951), so the PoC confirms the running operator actually serves the selector rather than gating on a pending bump. The same gate must be re-cleared for MariaDB (`MariaDB.spec.replicas`) when it lands, and is precisely what Redis/MongoDB fail — they expose no `scale` subresource at all. This gates everything else.
...
- **Migration (dev cluster, CNPG):** exercise the two-phase enable on a running multi-instance cluster and assert it does **not** collapse to 1 instance; then drive load and confirm the HPA scales `.spec.instances`, reads route to `<release>-ro`, and Flux does not revert. Exercise the disable path and assert it does **not** shrink the live cluster to the default `replicas`. Also exercise a **chart-version upgrade of an autoscaling-active cluster the HPA has grown**: assert a bump whose rendered seed is unchanged leaves `.spec.instances` untouched, and that a bump which does move the seed rebases only as the floor-change caveat documents (bounded, never to 1) — the platform-upgrade write path from [decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md). Also exercise a **restore of an autoscaling-active cluster** (in place and into an existing autoscaling-active app): assert the restored cluster comes up at the seed — never below the quorum floor and never at 1 — and that the surviving HPA re-scales `.spec.instances` back to load once recovery catches up.
...
- [0001. The autoscaler seed survives because the tenant HelmRelease applies client-side](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md) — why the original §3 "omit the field" was replaced by a constant seed applied client-side, and why the client-side two-way merge — helm-controller's current default for unstructured objects — does not re-assert the constant against the live count on upgrade/steady-state reconciles (the install-with-adoption recovery path is the documented exception; see decision 0001).
...
4. **The root cause is self-imposed.** The autoscaler-vs-Flux conflict exists only because the chart declares the replica field and Flux force-owns what it renders. §3 dissolves it by rendering a constant seed and applying the tenant HelmRelease client-side, so Flux computes its patch from the previous-versus-new manifest and — under helm-controller's current two-way-merge default on the upgrade path ([decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md); the install-with-adoption recovery path is the documented exception) — never re-asserts the field against the HPA's live count on steady-state reconciles. Removing the declaration outright — the original §3, "omit the field" — proved unsafe on helm-controller v1.5.0 (see [decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)); the constant-seed-plus-client-side form is what actually makes the ownership problem disappear.
