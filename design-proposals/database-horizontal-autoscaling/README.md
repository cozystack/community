...
The seed is a constant, so every reconcile renders the value it rendered last time; under **client-side** apply helm-controller patches the CNPG `Cluster` (a CRD) from the previous-versus-new rendered manifest and — under its current default, two-way merge for unstructured objects ([decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)) — does not consult live state, so the unchanged seed produces no patch and the HPA — writing the `scale` subresource — keeps sole effective control of `.spec.instances`. Client-side apply is forced per application by `release.cozystack.io/helm-server-side-apply: "false"` on the `ApplicationDefinition`, because the platform's helm-controller (v1.5.0) defaults to server-side apply, under which a *rendered* field is force-owned by Flux and reverted from the HPA's live value on every apply. This is what settles the ownership problem — no marker annotation on the CR, no admission webhook, and no terminal-freeze conflict handling is needed.
...
- **Steady state after migration is correct.** The seed is a constant present in every render, so helm-controller's client-side patch — computed from the previous versus the new rendered manifest, and, under its current two-way-merge default ([decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)), not from live state — re-asserts nothing, and the HPA-set `.spec.instances` is left untouched. The one exception is a change to the seed's own inputs: editing `.Values.replicas`, `autoscaling.minReplicas` or `quorum.maxSyncReplicas` moves the constant, and that single reconcile does rebase `.spec.instances` to the new seed (never below the quorum floor). Stage `.Values.replicas` to the live count first — or use the two-phase `transition` flow — to rebase deliberately with no transient.
