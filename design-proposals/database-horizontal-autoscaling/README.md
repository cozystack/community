...
- **Dry-run / recommendation** — render the dashboard and alert rules and still render the `ScaledObject`, but paused in **both** directions via KEDA's per-direction pause annotations (`autoscaling.keda.sh/paused-scale-in` **and** `autoscaling.keda.sh/paused-scale-out`, both `"true"`): the HPA stays up and keeps serving its trigger metric, but its `desiredReplicas` is frozen to the current count, so behavior can be observed — read the recommendation off the read-load panel (`desired = 1 + ceil(load/target)`) rather than the frozen desired-replicas line — before actuation is enabled. This is deliberately **not** KEDA's full `autoscaling.keda.sh/paused` annotation, which deletes the HPA and would leave no recommendation to observe.
