...
The seed is a constant, so every reconcile renders the value it rendered last time; under **client-side** apply helm-controller patches the CNPG `Cluster` (a CRD) from the previous-versus-new rendered manifest and — under its current default, two-way merge for unstructured objects ([decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)) — does not consult live state, so the unchanged seed produces no patch and the HPA — writing the `scale` subresource — keeps sole effective control of `.spec.instances`. Client-side apply is forced per `ApplicationDefinition` (i.e. per kind — it reaches every Postgres, see [decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)) by `release.cozystack.io/helm-server-side-apply: "false"`, because the platform's helm-controller (v1.5.0) defaults to server-side apply, under which a *rendered* field is force-owned by Flux and reverted from the HPA's live value on every apply. This is what settles the ownership problem — no marker annotation on the CR, no admission webhook, and no terminal-freeze conflict handling is needed.
...
4. **The root cause is self-imposed.** The autoscaler-vs-Flux conflict exists only because the chart declares the replica field and Flux force-owns what it renders. §3 dissolves it by rendering a constant seed and applying the tenant HelmRelease client-side, so Flux computes its patch from the previous-versus-new manifest and — under helm-controller's current two-way-merge default ([decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)) — never re-asserts the field against the HPA's live count. Removing the declaration outright — the original §3, "omit the field" — proved unsafe on helm-controller v1.5.0 (see [decision 0001](decisions/0001-client-side-apply-preserves-the-autoscaler-seed.md)); the constant-seed-plus-client-side form is what actually makes the ownership problem disappear.
