...
- **Implementation update (2026-09-02 PoC) — the SSA handoff is resolved by client-side apply, not by omitting the field.** The platform's helm-controller (v1.5.0) defaults to server-side apply, and the PoC settled the open question against the omit design on two counts: (1) a field the chart RENDERS is force-owned by Flux under SSA and reverted from the HPA's live `.spec.instances` (`/scale`) value on every apply; (2) the omit handoff relies on the HPA owning `.spec.instances` before the phase-2 apply, but the HPA claims the `/scale` field-manager only when it actually writes (`desired != current`) — so a cluster idle at its floor (`desired == current`) never writes, never owns the field, and the phase-2 omit prunes it, whereupon CNPG defaults it to 1 (quorum-fatal). That prune is unavoidable chart-only and would need a runtime actor this design deleted. The implementation therefore does NOT omit under active autoscaling: it renders a constant seed `instances: max(replicas, effectiveMin)` and forces CLIENT-SIDE (three-way-merge) apply for the tenant HelmRelease via a per-app annotation (`release.cozystack.io/helm-server-side-apply: "false"` on the ApplicationDefinition), under which the constant render is a merge no-op and KEDA's live value survives — deterministic and safe across enable / disable / dry-run with no handoff race, and uniform for the multi-engine roadmap. This supersedes §3's "omit the field" on the apply-strategy detail only; the ownership goal (Flux does not contend with the HPA for the field) is unchanged. Validated live on dev9: under SSA a rendered field is force-reverted from the `/scale` value on the next apply, and under the forced client-side apply the constant seed is a no-op that preserves the `/scale` value.
...
- ~~The two-phase enable/disable order is committed in §Upgrade; the one detail left for the PoC is whether the phase-2 field handoff rides on Flux/helm-controller SSA field-manager release (preferred) or needs the scale-subresource-pin fallback.~~ **RESOLVED by the PoC (2026-09-02) — see the "Implementation update" note in §Upgrade.** The chart-only omit handoff is NOT safe on the current platform, so the implementation keeps a constant seed and forces client-side apply per app; details below.
