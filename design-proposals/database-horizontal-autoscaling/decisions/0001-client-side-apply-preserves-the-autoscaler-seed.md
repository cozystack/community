...
- One residual write path remains under client-side too: because the two-way patch propagates any change between the previous and new *manifest*, anything that moves the rendered seed rebases `.spec.instances` in that single reconcile (never below the quorum floor, never a collapse to 1). Two triggers: a tenant edit to the seed's inputs (`replicas`, `autoscaling.minReplicas`, `quorum.maxSyncReplicas`), and — less obvious — a platform upgrade that changes how the seed itself is rendered (the `effectiveMin`/seed helper or a schema default), which rebases an HPA-grown count on the next apply even with no tenant edit. This is documented as the floor-change caveat in the chart's `values.yaml` (with a suspend-and-pin recipe for a deliberate rebase) and in §Upgrade (stage `.Values.replicas` to the live count, or use the `transition` flow); the platform-upgrade trigger is called out there too so an autoscaling-active chart bump is staged, not applied blind.
