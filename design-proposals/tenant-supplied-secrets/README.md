...
External Secrets Operator moves from optional to required in the platform bundle, and its chart is bumped to a current release. The central OpenBao from cozystack/cozystack#4177 becomes required in the PaaS bundle, at one replica on single-node installs.
...
- **Two new required components on upgrade.** External Secrets Operator and the central OpenBao install on the next platform upgrade. Neither touches existing tenant objects; the tenant chart renders the `SecretStore` and ServiceAccount on its next reconcile, and the OpenBao reconciler creates namespaces and roles for existing tenants.
...
2. **Required components.** External Secrets Operator bumped and moved into the required bundle; the central OpenBao from cozystack/cozystack#4177 lands and moves into the required PaaS bundle.
3. **Seam and store.** The tenant chart renders the `SecretStore` and ServiceAccount; the OpenBao reconciler provisions per-tenant namespaces, roles and policies; the cluster-wide override exists; tenant identities can authenticate to OpenBao.
4. **First consumers**, settling the reference shape in the process: VM import, which has no generation fallback and is the clearest win; MariaDB, whose operator takes a reference natively; and the site-to-site gateway, which is waiting on it.
5. **Remaining engines**, one pull request each, in the order the SecretRef proposal's engine analysis suggests. Each conversion decides its own inline-field deprecation.
6. **Tenant cluster consumption**: the per-cluster auth method in the OpenBao reconciler, the egress rule and CA delivery, and the `externalSecrets` addon on the managed Kubernetes application.
7. **Per-tenant override**, once the seam has been exercised by at least one consumer in production.
...
- **A write-only `TenantSecret` API version**, the user secrets API proposal's phase 2. Solves the write path inside the aggregated API with no new components, but leaves the platform holding every supplied secret in etcd with no audit beyond the apiserver's, offers the tenant no read-back, and cannot be pointed at a store the tenant already runs. Rejected in favour of a store the platform does not have to be.
- **Granting the existing `tenantsecrets` write verbs and stopping there.** The registry already exists and refuses to update objects it does not own, so the grant is cheap and safe. Rejected as the design because every supplied secret becomes projected to the `use` tier, and because it is a second write path once a store exists. It remains available as a stopgap if the store slips.
