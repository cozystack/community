...
- **Builds on** community #35 (IP addresses as a first-class resource) and its implementation, the address-controller (API group `local.sdn.cozystack.io/v1alpha1`). That proposal is the **allocation half**: what an address *is* — held, moved, quota'd, enumerated. This proposal is the **exposure half**: what an address is attached *to*. The two meet at exactly one point: an attachment either names an existing `IPAddressClaim` or mints one, and consumes it via the substrate's Service annotation contract (`local.sdn.cozystack.io/ip-address-claim`). The per-class drivers own everything below that annotation — pinning, announcement, association tracking. **The kind lives in #35's group family** — `local.sdn.cozystack.io`, the exposure half beside the allocation half, `SecurityGroup`'s networking family reached through the CRD-served `local.` prefix rather than the aggregated bare `sdn` — see *User-facing changes* for why that prefix is load-bearing. The attachment model stays inside #35's settled cardinality rule: N addresses on one application are N attachments, each rendering its own Service consuming its own claim — several Services each with one claim, which #35 blesses — never several claims on one Service.
...
- A tenant can attach one or more public addresses to a virtual machine — with whole-IP (1:1 NAT) semantics available for one of them — the live requirement the single legacy toggle cannot express.
...
- **Several whole-IP addresses on one VM.** Whole-IP is 1:1 NAT between one external address and one workload interface; the default is **one whole-IP Service per application**, and today's datapath enforces exactly that (see *Design*). Widening is deliberately left open for a datapath with stateful NAT and symmetric flows — a CNI capability, with cozyplane the expected first candidate (see *Open questions*). It is the same case #35 defers as *several owned addresses on one consumer*.
...
**VM datapath modes.** A vm-instance's tenant-facing Service is headless with a single sentinel port (`65535`) until exposed, so there is nothing to mirror: a VM attachment must set `ports` or `method` in the `loadBalancer` member (neither → `Provisioned=False` with a reason). `method: WholeIP` renders the existing cozy-proxy contract — the `service.kubernetes.io/service-proxy-name: cozy-proxy` label, the `networking.cozystack.io/wholeIP: "true"` annotation, and the sentinel port when `ports` is absent. `method: PortList` renders the same label with `wholeIP: "false"`, `networking.cozystack.io/allowICMP` from `allowICMP`, and requires `ports`. These are byte-for-byte the objects vm-instance's chart renders under `externalMethod` today — the datapath already exists; only this render was missing, and the enum deliberately reuses the chart's `externalMethod` vocabulary. **Whole-IP is 1:1 NAT, so it presumes a single backend**: the mapping is between one external address and one workload interface address, so an endpoint resolving to more than one backend has no interface to map — its external address is simply a load-balancer address, and `WholeIP` on such an endpoint is refused (`Provisioned=False`), not half-applied. **Default: at most one whole-IP Service per application**, counting a legacy `externalMethod: WholeIP` chart Service. This states the datapath's actual shape, not a caution: cozy-proxy translates statelessly — per-packet source/destination rewrites through strictly one-to-one address maps — and a second mapping for the same workload replaces the first in both directions, so a second whole-IP address today would not be ambiguous but dead. The controller refuses it with `Provisioned=False` (cross-object state, so a condition, not admission). Widening the default is left open for a more capable datapath — see *Open questions*.
...
- Admins gain nothing to configure beyond what #35 already gives them (classes and pools); attachments are quotable with a stock `ResourceQuota` (`count/endpointattachments.local.sdn.cozystack.io`), though the scarce resource — addresses — is already bounded at the claim. The quota claim works because the kind is **CRD-served by the kube-apiserver**, where ResourceQuota admission runs (the existing `count/certificates.cert-manager.io` pattern) — which is exactly what the `local.` prefix marks in #35's sdn family. The bare `sdn.cozystack.io` group, served by the aggregated `cozystack-api` as `SecurityGroup` is, never sees ResourceQuota admission and would silently drop quota support; putting the kind in `local.sdn.cozystack.io` encodes the CRD-served requirement in the group name itself, so a later group tidy-up cannot lose it. This is load-bearing beyond quota: incarnation-anchored GC by `ownerReference` (see *Lifecycle*) relies on the same real storage that only the CRD-served path provides.
...
- **`method: WholeIP` on an endpoint resolving to more than one backend** → refused with `Provisioned=False`; 1:1 NAT has no meaning against N interfaces, and the address would be a plain load-balancer address mislabeled.
- **A second whole-IP Service for one application** (a second `method: WholeIP` attachment, or one beside a legacy `externalMethod: WholeIP` Service) → refused with `Provisioned=False`; the datapath's one-to-one NAT maps admit a single mapping per workload (see *Design*, *Open questions*).
...
1. **Widening at-most-one whole-IP per application, when a capable CNI appears.** The shape of the widening is known in outline: with stateful (conntrack-based) NAT, reply traffic leaves through the address its connection arrived on, so secondary whole-IP addresses become ingress-plus-replies for free, and only unsolicited egress needs a designated primary mapping — the default-route analogue. Stateful NAT requires request and reply to traverse the same translating node (flow symmetry), a property today's stateless per-packet maps deliberately avoid depending on — so this is a CNI/datapath capability, with cozyplane the expected first candidate. What remains to design when such a datapath lands is only how the egress-primary is designated (an explicit marker on one attachment, or defaulting to the legacy Service's mapping). This is the datapath edge of #35's deferred *several owned addresses on one consumer* case.
